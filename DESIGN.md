...
**Optical tracking is graded by size, the way a native type stack grades it**, and it is baked into the ramp tokens rather than repeated at call sites. Display and the two largest headings track tight (`text-display` −0.03em, `text-2xl` −0.018em, `text-xl` −0.01em); reading sizes (13–18px) stay neutral, because Geist reads true there and tightening body text hurts it; the two micro sizes track *open* (`text-2xs` +0.01em, `text-10` +0.02em) so uppercase micro-caps do not jam shut. A call-site `tracking-*` utility still wins over the token — it is emitted after the size utility — so per-instance intent is preserved; the tokens are only the default. This replaced the flat `-0.015em` letter-spacing that the old global `h1/h2/h3` rule applied to every heading regardless of size.
