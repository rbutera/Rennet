...
| Freshness | One verdict per repo snapshot, up to a fixed cap, then a rollup tail (`… +N more repos — X fresh / Y stale`) so large multi-repo reviews stay under the byte ceiling |
