...
9. [#44 — command registry v1: conflicts, overrides, menu bar](https://github.com/rbutera/rennet/issues/44) — **delivered**: the shipped `buildCommands` registry now reads every stable command's title, group, and default chord from one exported `COMMAND_CATALOGUE` (its output stays byte-identical — the existing `commands.test.ts` is the positive control), and the palette, key dispatch, the settings Keyboard section, and the application menu all derive from it. **Persistent user overrides** ride an additive-optional `keybindings: Record<commandId, chord|null>` on the shipped `GlobalConfig` (`~/.rennet/config.json`), written through the same malformed-refusing `updateGlobal` path `setAppearance` uses via a new `settings.setKeybinding` command (string sets, `null` unbinds, omitted resets). Overrides are applied **at dispatch**: the two previously-hardcoded keydown sites (`app.tsx` window chords, `workspace.tsx` canvas keys) match through the registry over effective bindings, so a remapped chord runs the command and the old chord stops — the label never lies. **Conflicts are disclosure, never a block** (Rule Zero): a pure `findConflicts` reports every chord claimed by more than one command; the palette and the settings rows both name the collision; a conflicting write is accepted and persisted, first click, no confirmation, last match wins at dispatch. The **application menu** is projected by the renderer (`menuTemplate` → `menu.update` IPC) and built in MAIN via `Menu.setApplicationMenu`, labels/accelerators from the same catalogue, an out-of-context command disabled not hidden, item clicks routed back as command ids the renderer runs through the same handlers; command accelerators are display-only (`registerAccelerator: false`) so a chord fires exactly once, and `window.removeMenu()` is gone. On the override store: delivery order put this "after settings, which owns the override store", but `add-settings-v1` is itself unshipped and explicitly defers the store to #44 — so the override field lives on the shipped `GlobalConfig` now, and when the settings ladder lands it registers a `keybindings` global-layer key over this same field with no migration. **Deliberately not built** (no live consumer): cmdk/tinykeys adoption (the shipped registry already does their job), key sequences, macros, per-repo keybinding layers.
