...
10. Deferred tier, in dependency order as appetite allows: ~~[#25](https://github.com/rbutera/rennet/issues/25) Codex adapter~~ — **delivered**: `CodexAdapter implements HarnessPort`, the second real harness slot and peer of the Claude adapter. Transport verdict (the one deliberate divergence from the issue's letter, argued in the change's design): the adapter speaks `codex exec --json` behind an injected `CodexTurnTransport` seam — the mirror of `ClaudeQueryFn` — NOT the `codex app-server` JSON-RPC protocol. Evidence: every live `HarnessPort` consumer is single-turn (create → send → drain → close), the installed `codex` 0.146.0 labels `app-server` `[experimental]` and its shape has already drifted, and the Rule Zero amendment struck the approval apparatus that was app-server's main structural requirement. `codex exec` covers everything the acceptance criteria still need at exactly the capable-by-default posture (`--dangerously-bypass-approvals-and-sandbox`, real repo cwd, no gating). The app-server transport lands behind the same seam when steering or thread-resume is actually consumed. The desktop now resolves the `orchestrator-chat` council seat across both adapters: a Codex-selected turn reaches the injected transport and the same live canvasOps backend over loopback. Shipped with it: the **cross-adapter conformance suite** (`@rennet/core`, pure over `HarnessPort`) — one catalogue whose passing checks derive each capability flag through `buildCapabilities`, hermetic-by-default with a per-check refuting control and gated `.real` runs earning the outer layers only on a full expected pass/fail match; **testedRange** derived from a committed artifact (Claude keeps the permitted migration seed; Codex remains absent until its first genuine full-match real run); and **canvasOps@2 as an external loopback streamable-HTTP transport** so a codex session reaches the identical descriptors with no `if (harness === X)` branching. Codex token usage does not certify context-window size, which remains false. Struck approval scope is honored — no consent apparatus, not built, not tested for. Host locus only (WSL codex is #334's seam). This unblocks → [#41](https://github.com/rbutera/rennet/issues/41) cross-harness adjudication with seeded ground truth → [#26](https://github.com/rbutera/rennet/issues/26) omp slot; [#183](https://github.com/rbutera/rennet/issues/183) verify-ui independently. ~~[#26](https://github.com/rbutera/rennet/issues/26) omp slot~~ — **delivered**: `OmpAdapter implements HarnessPort`, the third real harness slot (R23), riding the #25 generalisation rather than rebuilding it. Transport verdict (argued in the change's design): `omp --mode rpc` line-delimited JSON behind an injected `OmpTurnTransport` — the third instance of the `ClaudeQueryFn`/`CodexTurnTransport` seam — NOT `omp acp`, whose distinguishing feature is a `session/request_permission` write-gating protocol (approval apparatus Rennet does not build; Rule Zero). The wire mapping stays inside the RPC subset `pi` shares. Capable by default (`--auto-approve`, `--no-session`, real repo cwd; the prompt is a stdin RPC command, never a positional). **Discovery is Bun-aware**: omp is a Bun-executed TypeScript entry point, so discovery proves both `omp` and a runnable `bun` (`~/.bun/bin` first), and an omp-present/Bun-absent slot degrades with a reason that NAMES Bun and still reports the resolved omp path ("found omp but not Bun"), never a first-spawn crash. **The honest constraint**: no turn has ever been executed against omp, so every wire shape is `.d.ts`-derived — the decoders are tolerant and passthrough-by-default, the hermetic fakes model only documented shapes, and the descriptor is evidence-derived. The default gate runs the conformance suite against an in-process fake (zero spawns, zero spend) and caps at `implementedByAdapter`; the outer layers and the recorded tested range are earned ONLY by the gated real run (`RENNET_LIVE_OMP=1`) on a full expected-matrix match — so **every capability flag stays false and `harness-tested-range.json` has no omp entry until that first real run** (the Codex precedent). The orchestrator seat serves omp through the same external loopback canvasOps@2 MCP transport Codex uses (identical contract, no harness conditional), with a deliberately minimal selection policy: omp serves **only when neither Claude nor Codex is installed**. **Deliberately cut** (Rule Zero / YAGNI): no `pi` discovery or slot (compatibility is a wire-subset discipline, not a fourth binary), no council-table extension (`scenarioFor` still ignores omp), no ACP / sandbox / read-only posture, no steer/resume/fork consumers, no harness-picker UI.
