...
11. ~~Polish sweep ([#316](https://github.com/rbutera/rennet/issues/316), [#65](https://github.com/rbutera/rennet/issues/65), [#89](https://github.com/rbutera/rennet/issues/89), [#92](https://github.com/rbutera/rennet/issues/92), [#75](https://github.com/rbutera/rennet/issues/75), [#223](https://github.com/rbutera/rennet/issues/223))~~ — **delivered** (four fixes, two evidence closes): a per-review **intelligence session** in MAIN now keys ONE `InvocationBudget` and ONE hypothesis promise by `(reviewId, activePatchsetId)`, so the canvas and flagged flows share a single per-review-turn ceiling instead of each spending the hypothesis on its own — a reattach (new patchset) re-derives and resets it (#316). The orchestrator **primer bounds B2/B3**: per-repo and per-canvas lines are capped, then a deterministic rollup tail (`… +N more repos — X fresh / Y stale`, `… +N more canvases — aggregate counts`) keeps a 10-repo / 20-canvas review under the 4 KB ceiling instead of throwing, with the rolled-up rows still reachable via the tool surface (#65). The council **harness is no longer overridable** — the override type carries `model`/`effort` only and `harness` always derives from the resolved model's provider, so no input can name an incoherent model/harness pair (#89). The owned tokenizer gains a **per-grammar comment word boundary** (shell/yaml `#` needs a boundary, python does not) and **radix-correct number scanning** (binary/octal/hex each accept only their digit class; a malformed literal like `0b102` fails closed to plain; the exponent carries the `_` separator) (#92, items 1–2). The symbol-inspector **miss copy names the committed review range**, so a symbol living only in uncommitted local edits reads as outside the reviewed range, not nonexistent (#223). **Closed with evidence, no code:** #75 (`documentRejected` has zero producers/ledger/consumers — the instrument would precede its signal) and #223's working-tree overlay (committed range is the right review scope; no demand). **Deliberately not built:** #92 item 3 (JS/TS regex literals — a line-local `/` divide-vs-literal guess would mis-highlight division, a worse lie than the current one). Then [#85](https://github.com/rbutera/rennet/issues/85) — the full design and usability pass — remains the open closing milestone. The accumulated `styles.css` design-system debt belongs there.
