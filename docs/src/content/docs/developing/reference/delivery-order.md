...
10. Deferred tier, in dependency order as appetite allows: ~~[#25](https://github.com/rbutera/rennet/issues/25) Codex adapter~~ — **delivered**: `CodexAdapter implements HarnessPort`, the second real harness slot and peer of the Claude adapter. It uses the capable single-turn `codex exec --json` transport behind `CodexTurnTransport`, not experimental `app-server`; reaches the same canvasOps@2 backend over loopback; and consumes the shared evidence-derived conformance suite. Hermetic runs cap at `implementedByAdapter`; a gated real run alone earns outer layers and a tested-range artifact entry. Codex usage does not certify context-window capacity. Host locus only; WSL Codex remains #334's seam. This unblocks [#41](https://github.com/rbutera/rennet/issues/41) and [#183](https://github.com/rbutera/rennet/issues/183).
