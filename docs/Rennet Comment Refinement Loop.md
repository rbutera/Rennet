...
**Headline recommendation up front: model refinement as a lifecycle on the existing disposition — the raw draft is kept verbatim and forever, and the refined form is a fleet-emitted, validator-admitted `refinement@1` document that carries into the collation draft where the user reads it, edits it, or reverts to their own words. Refinement runs always, in the background, on the light tier; it never blocks the user's next disposition; clarification questions are the exception path (a `needs-clarification` verdict rendered into a per-disposition inline thread), not the default. The clarification thread is local machinery and is excluded from publish — paper is what leaves the machine, and the paper is the effective body. Ship slice A (no thread: draft → background refine → the draft canvas → publish/handoff) first; it already delivers the whole "messy in, clean out" promise.**
...
interface ThreadMessage {
  messageId: string
  author: 'user' | 'refiner' | 'orchestrator'
  body: string
...
### 1.2 The two rules that keep authorship straight

Refinement puts agent-authored *text* where the user's words were, so two things have to hold:

1. **The draft is kept, verbatim, forever.** No agent edits, rewrites, or deletes `draft.body`. It is the permanent record of what the user actually said — the audit trail for "what did I write vs what got posted", the thing the strikethrough on the draft canvas shows, and the input every re-refinement starts from.
2. **The user reads what will publish, in their words or the refiner's, before they sign.** The refined body arrives on the collation draft next to the raw one; the user takes it, edits it, or reverts to raw, and the sign is where their authorship lands. Once published it is history (append-only, R32).
...
- **`drafted` is a complete, publishable disposition.** Refinement is an enhancement pass over a finished act. If the refiner never runs (model down, nothing came back), the disposition settles raw — the loop failing means *worse prose*, never *lost review work*.
...
### 1.4 Read state and the automatic transitions

⭐ **Read state attaches at `DispositionDrafted`.** Read is action-defined (OQ4 / correction 5), and the action is the user judging the code — which happens when they write the draft. A pending refinement must not leave code "unread" that the user has already judged; otherwise the totality guarantee would report false unread residue whenever the refiner is slow.

Two transitions settle without the user doing anything:
...
- **Verdict `no-change`** (the raw text was already clear and correct): the engine settles the disposition with `basis: 'raw-no-change'`, and the text that publishes is byte-identical to what the user wrote.
...
The **orchestrator sits beside the loop**: `canvas.thread(dispositionId)` (already specced in `canvasOps@2`) lets it read any clarification thread plus the current refined form, so it can answer "what's outstanding on this hunk?" honestly — and it can speak in the thread, since the user is reading the same words and holds the sign.
...
| **Not in its input** | The user↔orchestrator conversation. It emits a document; the engine does the rest |
...
- **No bound on rounds** — the refiner asks at most one question per run, and the escape hatch is always visible. In practice the draft canvas (§4) ends every loop that conversation didn't.

### 3.2 What the thread is not

- **Not published. Ever.** Only the effective body leaves the machine. The thread joins private events in the excluded-from-publish class (Rennet Contracts and Rulings: "private events structurally excluded from publish, proven by noninterference tests" — the thread gets the same test). The prepare step (§9.2) records canonical outbound bytes; thread bytes are simply never among them.
- **Not the main orchestrator conversation.** It is scoped to this one disposition. The orchestrator can read it (`canvas.thread`), speak in it, and carry it into the main conversation, but the thread stays anchored to its hunk.
...
## 4. The draft canvas: adjudication at any altitude

The refinement loop's adjudication surface is the **collation draft canvas** ([[Rennet Collation Draft Canvas]], accepted 2026-08-08) — still yours, still editable — and the frozen **paper** downstream is sign-only, carrying the verdict derived from the dispositions and overridable by the user. The roll-up/zoom principle dictates the form:

- The draft lists every disposition **showing its effective-body candidate**: your edit where you made one, the refined text where you left it, the raw draft where refinement failed or was bailed. Per row, a compact **raw → refined toggle/diff** — zoom in to see exactly what changed, zoom out to trust the roll-up.
- **Bulk adjudication is one act**: "Accept all refined versions (12)" — with per-row opt-out, per-row edit, per-row use-raw. Approve the roll-up or partials of it; the user picks the altitude.
- **Unsettled rows are shown plainly, and they never refuse the sign.** The draft says how many rows are still on the refiner's wording; the user signs when they choose, and what they signed is what goes.
- `needs-clarification` threads still open at sign time render as rows with the question visible and two affordances: answer now, or post-as-written. Nothing is ever silently dropped or silently posted raw.

### 4.1 Destination A: someone else's PR (publish)

- The **prepare** step records the effective bodies as the canonical outbound bytes (§9.2 unchanged in shape). The GitHub review posts the clean text as the user's own comment — first person, their voice, their identity. ⭐ **No AI-attribution markers on posted comments** (consistent with the frozen "no AI-attribution trailers, ever").
...
- Everything downstream is Rennet Contracts and Rulings §2.1 unchanged: batched bundle → new patchset → delta re-review; approved unchanged hunks stay approved.
...
- Refiner as one light-tier call; verdicts `refined`/`no-change` only (a would-be clarification degrades to `refined` at `confidence: low` — visibly marked on the draft).
- Adjudication **on the draft canvas only**: raw→refined diff per row, bulk accept, per-row edit/use-raw. Both destinations.
- No inline thread, no re-runs, no orchestrator involvement.
- *This is "write it messy → clean version lands on the PR", complete.* Cut line chosen because the draft canvas already exists as a surface and the validator already admits documents — slice A is mostly one doc type, one task trigger, and one column.
...
**Slice C — depth:** `alternatives` (different approaches, rendered as a choice), evidence-into-handoff-bundle, orchestrator thread participation, per-cohort bulk refinement summaries, E-series measurements (below).

**Measure before building past B** (same empiricism as the context-access doc): (E-r1) acceptance rate of refined bodies unedited — the loop's whole justification is this number being high; (E-r2) needs-clarification rate — if >20% the refiner is under-informed (feed it more context) or over-cautious (tune the verdict threshold); (E-r3) draft-time vs inline adjudication ratio — decides how much inline UI slice B actually needs.
...
2. **Canvas Paradigm L2**: L2 grows the raw/refined/effective lifecycle. `canvas.read`'s already-written "raw draft + refined form" return shape is confirmed; L2's description should name the lifecycle.
3. **Publication contract (Contracts §9)**: the draft canvas gains the raw→refined column; canonical outbound bytes = effective bodies; the clarification thread joins the excluded-from-publish class **with a noninterference test** like private events have.
...
6. **`canvasOps@2`**: `canvas.thread` was written anticipating exactly this loop; the orchestrator speaking in a thread adds one scoped `thread.reply` op.
