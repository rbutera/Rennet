...
⭐ **Staging semantics (ratified 2026-08-07, R36–R38):** dispose == staged (a disposition IS staged the moment it is made; the forming destination is the staging area); withdraw == unstage (#17's "batch view" is the **staged view**); publish is all-or-nothing per signing act in v1 (ship a subset = withdraw first, then sign). The ordered journey these serve is [[Rennet User Journey]].
