...
- It stops only the owned daemon: the process claimed by `daemon.json` in the app data directory, regardless of whether the app or CLI spawned it. The graceful `rennet stop` path sends SIGTERM, records in-flight turns as `interrupted`, and clears the claim before the app exits.
- The claim is **health-verified before any signal**. `daemon.json` is a claim to
  verify, never proof: Quit probes `/healthz` and signals only a PID whose identity
  and port the probe confirmed. A stale claim for a dead or unrelated process is removed without sending a signal, so tray Quit cannot take down
  someone else's process.
- It exits even if the daemon does not confirm death within a bounded wait,
  recording the timeout in the log. The next launch verifies the claim again.
