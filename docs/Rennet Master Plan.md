...
| 2 | **Decisions** | queue | The calls only you can make. The **decision log is the angle's spine**: everything the author(-agent) decided, each with a reconstructed WHY marked as reconstructed; disposition triage **evidenced / mechanical / contestable**; only contestable items consume slots in the **capped queue, which is a surfaced view** (salience-ranked, cap truncates the view, never the emission). Ratified in principle. |
...
**The engineering thesis (ratified verbatim):** the clever/novel engineering is (1) **a surfacing format/DSL** any coding agent uses to surface the right areas — validated deterministically, rendered by us, published as an open spec (RSP, Apache-2.0) — and (2) **model-tier routing**: heavy models where they reason (decomposition graph, decision reconstruction, adjudication), light models where they fetch (test mapping, labels, noise summaries), deterministic code wherever a tool can be 100% right. Plus the ratified third leg: **the instruction layer** — versioned base instructions shipped with the app, user guidance layered through the settings ladder, the exact assembled prompt always inspectable. The composed property: **a wrong model produces an invalid document, not a wrong review.**
...
| R2 | Bundled/linked `@anthropic-ai/claude-agent-sdk` (arch D12/B4/B18, adapter plan §2.1) | **Retired.** The SDK is proprietary and AGPL-incompatible; the Claude adapter is a **clean-room process-per-turn wrapper** over the user's installed `claude -p --output-format stream-json --include-partial-messages --resume <id> --fork-session --json-schema`, tolerant runtime decoders, never importing the SDK or its types. Deletes the nested-binary notarization trap and the bootstrap CLAUDE.md's "one compiled artifact" line: the stack is **zero compiled artifacts**. Adapter plan §2.1's mapping table survives as conceptual guidance only; its SDK transport does not. | Adjudication verdict; [[Wingman Distribution and Licensing Plan]] §0.2; hub decision |
| R3 | Protocol/types inside AGPL `core` (arch D1 subpath exports) | **Split at the package boundary**: `packages/protocol` **and** `packages/types` ship **Apache-2.0**; core/adapters/ui/desktop are AGPL-3.0-only; the proprietary mobile app consumes the two Apache packages as published npm packages only. Protocol may depend on types; neither imports anything else in-repo (CI-enforced). | Distribution plan §0.1; hub decision |
| R4 | Instructions licensing: DSL plan §6.1 puts `packages/instructions` **AGPL** while the DSL itself is Apache | **Upheld, no conflict.** The DSL (schemas, anchor grammar, validator codes, conformance corpus) is the interoperability surface → Apache in `packages/protocol`. Base instructions are product voice, change weekly, and are deliberately NOT part of the spec (DSL plan §7: a third party implementing RSP writes their own instructions and still interoperates). Instruction *digests* appearing in Apache-defined provenance blocks is data flow, not linking. CI rule: `instructions` is never a dependency of protocol/types/mobile. | DSL plan §6.1 + §7, read against distribution plan §0.1 |
| R5 | AGPL-3.0-**or-later** (bootstrap plan §0, CLAUDE.md draft) vs AGPL-3.0-**only** (distribution plan) | **`AGPL-3.0-only`.** `-or-later` hands the FSF a unilateral relicensing right, which is incoherent next to sole-author dual licensing. Bootstrap's licence table and CLAUDE.md draft are corrected at regeneration. | Distribution plan §4.1; hub decision |
...
- Licence split per R3/R4/R5: Apache-2.0 `protocol`+`types`; AGPL-3.0-only everything else open; proprietary mobile via published packages only. SPDX headers, REUSE lint, protocol/types import-nothing rule.
- Rai is sole copyright holder: **no outside code contributions before the policy lands (R22), no AI-attribution trailers, ever.**
- Never link a proprietary SDK (R2). Never read a credential (adapter plan D6). Never bundle a harness binary.
...
3. `angles.decisions.maxItems` — measure ten real PRs uncapped first.
4. Skimmed-vs-read trigger semantics (arch OQ4).
5. Quote-match rejection rate on real diffs; normalisation width.
6. Salience vs deterministic blast-radius ranking for the decisions queue.
7. `done` for a spec-only changeset; spec-drift view across patchsets; how far spec derivation may reach (falsifiability rule, unvalidated).
8. Claim identity: does semantic matching need a model call at all.
9. Per-thread harness switching: one session per harness vs replayed context (leaning fresh-per-harness).
10. Monorepo sub-package settings (nearest-ancestor context first; nested repo files if insufficient).
11. Config schema versioning detail (needed before the first key rename).
12. Editor `auto` re-resolve-per-launch vs bind-once; running-bundle precedence.
...
16. Windows/Linux signing cost (research only); docs/ licence choice; commercial-licence trigger wording.
