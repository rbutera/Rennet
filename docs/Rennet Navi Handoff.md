...
- **The canvas contract.** Five canvases + blast-radius overlay; L0–L3 layers; the user authors and signs what publishes. [[Rennet Canvas Paradigm]].
...
- **LSP**: inline definition chunks below the line (opaque code body, glass header, tier badge, breadcrumb cap 3), degraded-result detector + positive-control readiness probe load-bearing, definitions are context never coverage; **open-in-editor** above every diff.
- **Settings + instruction layer.** Eight-layer ladder (global → workspace → repo → changeset; personal vs shareable; provenance is the return type; context and guidance read at base ref for others' PRs). Versioned base instructions in `packages/instructions`; user guidance via `instructions.*` keys with `append` merge (R12); assembled prompt always inspectable; the contract is not configurable, the voice is.
...
- **DSL + model-tier routing.** JSON documents against versioned schemas in `packages/protocol`; agents never mint identity; quotes verified byte-for-byte; validator is the gate; deterministic/light/heavy routing with the <15s / <5-invocation budget as a CI-tested performance target.
...
9. **No auto-approve, no auto-comment, nothing another human sees without an explicit human action.** Product rule and repo rule.
