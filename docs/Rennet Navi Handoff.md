...
- **Decisions angle = the decision LOG.** Everything the author(-agent) decided, each with reconstructed WHY marked as reconstructed; disposition triage **evidenced / mechanical / contestable**; only contestable items occupy the **capped queue, which is a surfaced view** (salience-ranked; the cap truncates the view, never the emission).
...
- **Settings + instruction layer.** Eight-layer ladder (global → workspace → repo → changeset; personal vs shareable; provenance is the return type; repo files untrusted behind the trust gate; context and guidance read at base ref for others' PRs). The instruction layer rides the same ladder: versioned base instructions shipped in `packages/instructions` (AGPL), user guidance via `instructions.*` keys with `append` merge (ratified, R12), assembled prompt always inspectable; the contract is not configurable, the voice is; shared layers may never raise spend.
...
- **DSL + model-tier routing thesis.** JSON documents against versioned schemas in Apache-2.0 `packages/protocol`; agents never mint identity; quotes verified byte-for-byte; validator is the gate; deterministic/light/heavy routing with the <15s / <5-invocation budget as a mechanical CI-tested gate.
...
- **Licensing.** Apache-2.0 `packages/protocol` + `packages/types`; AGPL-3.0-only everything else open; proprietary mobile consumes the two Apache packages as published npm packages only; `packages/instructions` is AGPL (R4).
...
3. **Never link `@anthropic-ai/claude-agent-sdk`** or any proprietary SDK; never copy its types; never bundle a harness binary; never read a credential (R2).
...
Local repo, neutral name (`review-harness`), per the deferred-rename rule. LICENSE = **AGPL-3.0-only** (R5, not `-or-later`), pnpm workspace (`save-exact`, `strict-peer-dependencies`), Biome, shared tsconfig bases (`portable.json` with `"types": []` + `"lib": ["ES2022"]`), `packages/protocol` + `packages/types` skeletons, turbo graph, root `gate` script. Layout per Master Plan R21: `packages/{types, protocol, core, adapters, ui, instructions, tsconfig}` + `apps/{desktop, mobile}` placeholder + `scripts/` + non-workspace `spikes/`. [[Wingman Repo Bootstrap Plan]] §1–§3, §6 commits 1–6.
...
`LICENSES/`, root `REUSE.toml` (default AGPL-3.0-only, © Rai Butera), per-package `REUSE.toml` for Apache-2.0 protocol+types, two-line SPDX headers everywhere, `reuse lint` in the gate, plus the CI rule that protocol/types depend on nothing in-repo (types on nothing at all). [[Wingman Distribution and Licensing Plan]] §0.1, §4.1, B1.
...
Drop in the bootstrap plan's §4 CLAUDE.md corrected per Master Plan: AGPL-3.0-**only**; **zero compiled artifacts** (delete the "one compiled artifact is the Agent SDK binary" line — SDK retired, R2); DCO + CONTRIBUTORS grant instead of generic CLA; ForgePort not GithubPort (R24); six-angle v4 vocabulary; never-bare-FileDiff. `AGENTS.md` symlink. [[Wingman Repo Bootstrap Plan]] §4 as amended.
...
**55. Claude CLI clean-room adapter** — P0 — deps: 53, 54, 6.
...
One base instruction per v1 model-touching task (`decomposition.proposal@3.md` style), AGPL (R4), fixed skeleton (role, docType, anchor discipline, closed vocabularies, failure behaviour = emit residue/unclaimed/insufficient, never guess), never restating the schema. Versioned like schemas or the rejection log means nothing. [[Wingman Surfacing DSL and Model Routing Plan]] §6.1, D26.
...
**104. Decisions angle UI (the log + the capped queue)** — P1 — deps: 76, 100.
The decision log as the angle's spine; disposition triage rendered; only contestable items in the capped queue (salience-ranked view, "12 more below the line" honest truncation); each item carries reconstructed WHY marked as reconstructed and everything needed to discharge in place (shows + reach). Hub decisions-refinement (ratified in principle).
...
Two paragraphs: AGPL grants rights in the code, not the name or logo; forks rename; unmodified redistribution may use the name. ™ not ® (® unregistered is a UK offence). Registration itself: not now; trigger = first paid sale or first collision. [[Wingman Distribution and Licensing Plan]] §6, B10.
Done: file exists; no ® anywhere.

**136. AGPL §13 source link in About, shipped with pairing** — P3 — deps: mobile pairing (not in this backlog).
...
6. **Lawyer read of the AGPL analysis** (protocol split, §13, dual licensing, App Store precedent) before the repo goes public. [[Wingman Distribution and Licensing Plan]] final warning.
