...
> ⚠️ **RULE ZERO (CLAUDE.md, 2026-08-11) outranks this document and every ruling it cites.** No consent gates, no gates, no robustness for robustness' sake. Passages carrying a ⛔ SUPERSEDED marker are void where they conflict; the repo gate (`pnpm check`), the client-repo boundary, the no-AI-attribution rule, and the MIT licence are untouched.
...
- **The canvas contract.** Five canvases + blast-radius overlay; L0–L3 layers; **L2 (dispositions) is user-sovereign — no agent writes it**; the actor partition is structural, never prompt-enforced. [[Rennet Canvas Paradigm]].
  > ⛔ **SUPERSEDED 2026-08-11 by RULE ZERO (CLAUDE.md).** No consent gates, no gates, no robustness for robustness' sake. The canvases and layers stand; enforcing the actor partition by *withholding capability* does not — the agent may draft and write into the working draft, and the user authors and signs what publishes.
...
- **LSP**: inline definition chunks below the line (opaque code body, glass header, tier badge, breadcrumb cap 3), degraded-result detector + positive-control readiness probe load-bearing, definitions are context never coverage; **open-in-editor** above every diff with copy disclosure before the click.
  > ⛔ **SUPERSEDED 2026-08-11 by RULE ZERO (CLAUDE.md).** No consent gates, no gates, no robustness for robustness' sake. Open-in-editor just opens; the copy-disclosure step before the click is ceremony and goes.
- **Settings + instruction layer.** Eight-layer ladder (global → workspace → repo → changeset; personal vs shareable; provenance is the return type; repo files untrusted behind the trust gate; context and guidance read at base ref for others' PRs). Versioned base instructions in `packages/instructions`; user guidance via `instructions.*` keys with `append` merge (R12); assembled prompt always inspectable; the contract is not configurable, the voice is; shared layers may never raise spend.
  > ⛔ **SUPERSEDED 2026-08-11 by RULE ZERO (CLAUDE.md).** No consent gates, no gates, no robustness for robustness' sake. The ladder, provenance, and the inspectable assembled prompt stand; the **trust gate** on repo instruction files and the **shared-layers-may-never-raise-spend** money circuit are gates and go — read the repo's instructions and run.
