...
| ID | Current ruling |
|---|---|
| **R36** | Everything the review gathers is an ask: a typed message carrying an anchor, text, an intent, and an exit lane, with provenance back to the finding, comment, thread, or conversation it came from. Staging is the reviewer's act: it stages directly when the reviewer states the conclusion, and Rennet offers a one-tap pill when it infers one rather than staging on its own. Findings never auto-stage. |
...
| **R40** | Outbound documents are living drafts whose initial structure and model reworks belong to Rennet's rework workers. The reviewer may save a direct block edit or steer by conversation and span selection. A saved edit replaces the canonical durable block consumed by every exit; it never becomes a client-only shadow. Retired content is ledgered with its reason and stays restorable. Preview renders the exact payload that posts. |
...
| **R51** | A session's conversation is its T3 Code thread, briefed as Rennet's and holding Rennet's app tools. It answers on one model, routed by the council's `orchestrator-chat` job; there is no second-opinion mode and no separate orchestrator chat surface. |
