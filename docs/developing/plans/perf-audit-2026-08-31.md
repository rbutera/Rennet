...
**H1. `packages/adapters/src/project-context-reader.ts:80` — `loadFresh` re-reads + re-sha256s the entire project snapshot per `context.*` request.** `verifySnapshotIntegrity` walks every shard (readFileSync + sha256 each), then `materializeSnapshot` re-invokes the same unmemoized `load` per shard again + JSON.parse. ≈2×N_files sync reads per call; callers are per-request/per-slice (context.map/file/symbols; the knowledge-swarm caller measured here was removed with the context-map kill, 2026-09-01). Likely the single largest daemon CPU item on a big repo. Fix: memoize `load`/manifest keyed on (repoKey, baseOid).
...
**H2. Renderer is one unsplit 1.8 MB chunk.** No `manualChunks`, zero `React.lazy`/dynamic import anywhere in app-ui/client/renderer; wouter routes all eager. Shiki + 9 grammars sync-imported at module scope (`app-ui/src/syntax/shiki.ts:23`, deliberately sync for tokenization); `motion/react` eager for one welcome screen; react-markdown eager. And `sourcemap: true` in prod → **6.8 MB `.js.map` ships in the packaged app**. **Wave 4: the welcome/motion split and the sourcemaps are fixed; the shiki grammars are not — see fix-plan item 12 for the measurement and why.**

**M3.** `dist/browser` is a near-identical second copy of the renderer bundle (build cost + package size, not runtime). **Investigated, declined (Wave 4):** "byte-identical" was wrong — the two builds have genuinely different entries (`browser/entry.tsx` composes its bridge from `location`, routes on `browserHistory`, and intercepts `repository.choose`; `renderer/index.tsx` reads the preload, routes on `hashHistory`, and resolves a WSL target). They come out 0.8 kB apart only because `@rennet/app-ui` is almost all of both. The reason is now recorded in `vite.browser.config.ts` so the question is not re-asked.

**L4. Dead dep**: `@modelcontextprotocol/sdk` in `packages/adapters/package.json`, zero imports. Delete. **Done (Wave 4)** — with the correction that it is not free-floating: it is a required peer of `@anthropic-ai/claude-agent-sdk`, so removing the declaration changes no installed bytes under the hoisted linker, it just stops adapters claiming a dependency it does not use.
...
**Wave 2 — the quadratics (the "long session melts" class): LANDED on `perf/audit-waves-2-4` (2026-08-31) — items 6, 7, 8 in full; item 5 read-side only (stat-validated parsed-file cache; the append-only write format carries forward, see the item body).**
5. Ask-log store: in-memory projection + append-only writes (§3 H1 / §4 H4) — same pattern then applied to thread, transcript, board stores. **The READ side landed**: a stat-validated parsed-file cache removes the re-read, the `JSON.parse` and the zod walk from every call, across the ask-log, thread, transcript, session and board stores. **The append-only on-disk write format did not, and carries forward as a follow-up**: every mutation still serializes and rewrites its file whole, so a write is still O(n) bytes and a long session's writes are still quadratic. That is deliberate scope — changing the on-disk format is a persistence change under architecture contracts, not a caching one, and the whole-file temp-then-rename write is what makes these stores atomic today.
...
**Wave 3 — renderer scale (700-claim boards): LANDED on `perf/audit-waves-2-4` (2026-08-31), all three items.**
9. `Collapse` unmounts when closed; `memo()` on Section/FindingElement/Turn/DiffFileCard; `useMemo` RichText/useRangedThreads. **LANDED.** Collapse mounts on open and unmounts 200 ms after close (animation preserved; no `keepMounted` consumer needed — all four audited), and a collapse that mounts ALREADY closed schedules no unmount timer at all — the board's common case, where every folded section used to arm a timer whose only act was to set `false` on `false`. `COLLAPSE_MS` is exported, because `followBoardAnchor` has to wait out the same window before scrolling to a target whose section it just opened; a jump issued a frame after the toggle measured a section still growing from 0fr and landed short. `memo(Section)`/`memo(BoardElement)` with stable-identity derivations; `RichText` segmentation in one `useMemo`; `useRangedThreads` split scope/locate so a foreign quote-thread write derives nothing. `memo(Turn)` + module-scoped `renderRow` on the transcript; a settled streamed word collapses to a text node and the `word-in` keyframe no longer retains a `filter`, so revealed words keep no stacking context. Accepted trade: folding a section discards its children's own fold state.

    Stated precisely, because the test name used to overclaim it: the `useRangedThreads` split does **not** stop a quote-thread write reaching every mounted element. `useScopedThreads` still re-runs its filter over the whole `quoteThreads` record for each one — O(threads) per element, O(elements × threads) for the board — and what it buys is a stable array identity when that element's scope did not change. The identity is what then skips the expensive half: the anchor location, the disjoint decoration build, and the re-render of the highlighted prose. Removing the remaining per-element scan would mean indexing threads by target in the store, which is not in these waves.

    Follow-ups: transcript windowing (memo pass landed; the DOM still holds every turn). And one **pre-existing** observation found while reviewing this wave, not caused by it and not fixed here: `chat-data.ts` adds every `status: "streaming"` row from the reattach snapshot to `liveIds`, so reopening a session whose turn is mid-stream animates the whole `bodySoFar` as a fresh arrival rather than replaying it as a record. The line dates to `d5bd1f4d` (C07 cluster 1) and is byte-identical on `main`; a fix would have to seed `liveIds` only from events received after the initial reattach settles.
...
11. Bound `CommandCache`; session-list caching daemon-side (§4 H5). **LANDED** (session-list half shipped in Wave 2's store cache). `CommandCache` releases what unobserved entries hold — data dropped on the coldest abandoned mutable entries past 256 and immutable spans past 1024 (measured peak abandoned set: 37) — while observed entries never release and entry bookkeeping survives so supersede generations stay correct. Two holes in that bound were found in review and closed: the last unsubscribe of an immutable key now runs the immutable trim (previously it returned early, so more than a cap's worth of spans subscribed concurrently and then all closed were retained until some unrelated later read happened to trigger an eviction — or forever, if none did), and a release now bumps the entry's generation, so a fetch still in flight when its entry was released cannot reinstall the payload onto an entry the LRU has already stopped accounting for.

**Wave 4 — startup + size:**
12. Split renderer bundle (lazy welcome/motion, lazy shiki grammars); drop prod sourcemaps from the package; dedupe `dist/browser`. **LANDED in part.** The welcome wizard is `React.lazy`'d behind a `null` fallback in `routes/app.tsx`, which moves it and `motion/react` — its only importer in the whole renderer — into a 95.8 kB chunk fetched only when the wizard is elected; the startup chunk drops 1.96 MB → 1.86 MB (gzip 482 kB → 449 kB). Prod sourcemaps are off for the renderer, browser, server AND preload builds: nothing read them (no crash reporter, no `source-map-support`, and dev debugging goes through the dev server), and `forge`'s `prune: false` was shipping all of them. `dist/main/index.cjs.map` is the ONE that stays, because `scripts/smoke-packaged-app.mjs` reads its `sourcesContent` out of the asar. Net packaged `dist/`: 33.3 MB → 8.9 MB.
    **The shiki grammars did NOT move, and cannot cheaply.** Measured: shiki + the nine grammars are 775.6 kB raw / 119.7 kB gzip, 42% of the startup chunk — the single biggest slice. Deferring them needs an `await`, and `tokenizeLine` is contractually synchronous for three render-path consumers (`code-view`, `diff-view`, `code-block`) that must have tokens on the first render, so an async facade buys the bytes with a flash of unhighlighted code. Route-lazying `ReviewWorkspace` to carry them was measured too and moved 4 kB: the module is reachable from another eager edge, so rolldown emitted no separate chunk. A `manualChunks` vendor split was declined as ceremony — every chunk is a static import of the same entry, so all of it is still fetched and parsed at startup, and the caching it would buy is not something the Electron `app://` handler even sets headers for (§2 M4).
13. Drop dead `@modelcontextprotocol/sdk`; `sideEffects: false` + deep imports for the main bundle. **LANDED.** `@modelcontextprotocol/sdk` is gone from `packages/adapters` — no module imports it; it remains resolvable as `@anthropic-ai/claude-agent-sdk`'s declared peer, which pnpm's hoisted linker keeps at the workspace root, so this is lockfile honesty rather than bytes. `sideEffects` is now declared on all five server-side packages, and the Electron main bundle halves: 455.7 kB → 236.7 kB (gzip 126.2 → 68.1), with `graphology` and `execa` — pulled in for nothing but `defaultDataDir` and the daemon-supervision helpers — verified absent from the output where the baseline carried them. `server` declares its two process entries (`daemon-main.ts`, `cli-main.ts`) rather than a blanket `false`, because both run real top-level side effects: `dns.setDefaultResultOrder("ipv4first")`, the `net.setDefaultAutoSelectFamily*` pair, and `UV_THREADPOOL_SIZE`. Ceiling, stated honestly: the control did NOT prove that allowlist load-bearing — a blanket `false` produced a byte-identical server bundle, because rolldown preserves entry-module side effects either way. It is the truthful declaration, not a proven guard. The deep-import half of this item was declined: `@rennet/server`'s `exports` map has no deep paths, and `daemon-supervisor.ts` imports seven live symbols from the same barrel regardless, so tree-shaking was the whole lever.
    Not part of this item but found next to it: `daemon-main.ts`'s comment claims its dual-stack and thread-pool settings cover `rennet serve`. They do not. The CLI bundle's entry is `cli-main.ts`, which reaches `runDaemon` through `./daemon` and never imports `daemon-main.ts`; `grep setDefaultResultOrder packages/server/dist/rennet.cjs` returns zero, and did so before this change too. Pre-existing, unfixed here.
