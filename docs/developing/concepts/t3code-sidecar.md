...
`.rennet/context/<sessionId>/tool-results/` and the returned result naming that path. The
thread reads the file when it wants the rest — the same progressive disclosure the seats
get, applied to the answers rather than the questions.
...
The root and both layouts resolve off the settings ladder for the reviewed repository, as
does `workspace`; the four keys and their rungs are in
[Settings and setup](../guides/settings-and-setup.md#worktrees), and what a sibling means for
the round, the push and the land action is in
[Handoff and exits](handoff-and-exits.md#the-work-branch-reaches-the-branch).
...
session's **work branch** there — but on a thread of its own (see
[Rounds as threads](#rounds-as-threads) below), not the one the chat and handoff share.
