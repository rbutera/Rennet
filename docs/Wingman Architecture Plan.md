...
Audience: an autonomous implementation agent (Navi). This document is decision-complete where it can be and explicitly refinement-hooked where it cannot. Read [[Code Review Harness App]] first for product intent; read the stack note for package choices, which are **not re-litigated here**.
