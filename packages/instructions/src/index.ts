...
export const FINDING_VERIFICATION_CONTRACT: VerificationContract = {
  ...
  task: "For each finding you are given — its reference key, its severity, and its one-sentence concern — reproduce it or refute it. PREFER EXECUTED EVIDENCE: when you can run the code to make the concern happen (or to show it cannot), do that. When you reproduced a finding BY RUNNING something, put the EXACT command you ran in that verification's `command` field, verbatim and character-for-character as you ran it, and set the evidence to what it proved — the run is checked against the commands actually observed, so a `command` that was never run is dropped from the proof. Attribute each command to the ONE finding it reproduces; do not claim a command for a finding it did not exercise. REPRODUCE: name the concrete failure — the command whose output shows it, the failing test, or the exact lines that make the concern true. REFUTE: show, by running it or from the code, why it does not hold. If you can honestly do neither, return INCONCLUSIVE. Emit exactly one verification per reference key you were given, echoing that key unchanged. The exact JSON shape is enforced separately as a structured-output constraint you must satisfy; do not describe or restate it.",
  discipline:
    "Ground every verdict in evidence you actually have: what a command printed when you ran it, or the specific code you read. You are shown a file window to start from, but you are NOT confined to it — read more of the repository, and run it, when that is what it takes to know. The evidence is ONE line: for a reproduced finding, the command and its result or the lines that prove it; for a refuted or inconclusive one, the concrete reason. Only fill `command` for a finding you actually reproduced by running that exact command — never a command you merely intended to run or paraphrased. Do not soften a genuine bug into inconclusive to be safe, and do not upgrade a hunch into reproduced to look decisive.",
  failureValve:
