...
// narration). Each producer runs with its OWN invocation budget exactly as the
// production commands do (they are separate budget-gated user actions).
//
...
function summarize(metrics: readonly TurnMetric[]): void {
  ...
  line(`model(s): ${models.join(", ") || "unknown"}   apiKeySource: ${auth.join(", ") || "unknown"}`);
    ...
    async () => {
      ...
      if (METRICS_OUT) {
        writeFileSync(
          ...
          JSON.stringify(
            {
              target: { baseOid: BASE_OID, headOid: HEAD_OID, files: patchset.files.length, diffBytes: patchset.byteLength },
