  ...
  return async (input) => {
    ...
    try {
      session = await port.createSession({
        ...
        // Both observers consume normalized live frames. The SDK produces those
        // frames only when partial messages are requested.
        ...(input.onDelta === undefined && input.onEvent === undefined
          ? {}
