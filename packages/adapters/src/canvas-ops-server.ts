...
export const CANVAS_OPS_INSTRUCTIONS = [
  ...
  "The project (base branch, model-free): context.map (structure), context.file (one file's facts), context.novelty (the change vs the baseline). NAVIGATE THE CODE SYMBOLICALLY BEFORE DUMPING WHOLE FILES: context.overview lists a file's exported symbols so you learn its shape without reading it, and context.symbol resolves an exported symbol NAME to its definition site(s) so you jump straight to where something is defined. Reach for these to pull the symbol or lines you need — do not read an entire file to find one thing.",
