 ...
 * Claude one, but it maps the terminal frame to `{ status, body }` and DISCARDS
 * the SDK result frame's `usage` block — so every provenance the runners stamp
 * carries `ZERO_TOKENS` on the Claude path, and there is no baseline to measure a
 * future intelligence-core build's DELTA against.
 *
...
import type { HarnessPort } from "@rennet/core";
...
export function createInstrumentedRunTurn(
  ...
  return async function runTurn(prompt: string, attempt: number): Promise<HarnessTurnResult> {
    ...
    const finish = (result: HarnessTurnResult, usage: ClaudeTurnUsage | null): HarnessTurnResult => {
      collector.record({
        label,
