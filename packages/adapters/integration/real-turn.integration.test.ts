...
describe.skipIf(!LIVE)("live Claude real turn (opt-in; spends the subscription)", () => {
  it("round-trips structured output through the real SDK on the subscription path", async () => {
    ...
    console.log(
      `[real-turn] using claude at ${discovery.chosen?.path} (v${discovery.chosen?.version})`,
    );
    ...
    const apiKeySource = started?.kind === "session.started" ? started.apiKeySource : "MISSING";
    // eslint-disable-next-line no-console
    console.log(
      `[real-turn] apiKeySource=${apiKeySource} meteredWarning=${meteredWarning ? "YES" : "no"}`,
    );
    // eslint-disable-next-line no-console
    console.log(
      `[real-turn] outcome=${JSON.stringify(ended?.kind === "session.ended" ? ended.outcome : ended)}`,
    );
