...
export const NOISE_CONTRACT: PromptContract = {
  ...
  input:
    "Nothing about the change is sent to you inline. The context layer names the offered manifest file (`noise-offer.json` in this session's context directory): every hunk you may anchor to, each with its immutable `id`, its `path`, and its 1-based line ranges on the old and new side. Your working directory is the reviewed checkout, so read the lines of each offered hunk from `git diff` there. Anchor each churn item to exactly one offered hunk, written `rennet:hunk/<id>`. An id that is not in the offer is rejected at parse time, so never invent one, and group only churn whose lines you actually read — never code you did not open.",
  discipline:
