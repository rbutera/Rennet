...
An older caller may supply a `hostSchema`, a verbatim `worker.diff`, and require
a DraftBoard instead of the narrow envelope. Only on that shape, express the
same verified classifications as `round_outcome` items with changed-line
`code_ref` evidence read out of that diff.
