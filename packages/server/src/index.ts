...
// The `app_*` in-process agent tool surface (#465): built by iterating the command registry
// for `exposure.agent` rows. Exported so the orchestrator turn maps them onto the harness SDK's
// `tool()` when it constructs a turn (the whiteboard five stay HTTP MCP, untouched).
export {
  type AppTool,
  type AppToolDispatch,
  appToolName,
  buildAppTools,
} from "./agent-tools";
export {
