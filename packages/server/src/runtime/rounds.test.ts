...
describe("createRoundsRuntime", () => {
    ...
    const runtime = createRoundsRuntime(
      withFakeT3Seats(
        baseDeps({
          ...
          loadGenerationVersion: () => ({ generation: PREV_GEN, revision: 1 }),
          freezeGeneration: () => {
            order.push(`generation:${PREV_GEN.id}`);
            return freezeGeneration(PREV_GEN);
          },
  ...
  it("exposes both generations by id — the switcher drills back to the frozen predecessor (C15 2.3)", async () => {
    ...
    generationStore.save(priorWithBoards);
      ...
      const runtime = createRoundsRuntime(
        withFakeT3Seats(
          baseDeps({
            ...
            loadGenerationVersion: () => ({ generation: PREV_GEN, revision: 1 }),
            freezeGeneration: () => freezeGeneration(PREV_GEN),
            onGenerationTransition: () => {
              transitions.push("transition");
            },
            recordRound: () => {
  ...
  it("keeps a predecessor replaced after observation and publishes only successful freezes", async () => {
    const prior: Generation = {
      ...
      patchsetId: "ps-0",
      lensBoards: {},
      draftingReportBoardId: "attempt-a",
      status: "live",
    };
    const run = async (replaceAfterObservation: boolean) => {
      const directory = mkdtempSync(join(tmpdir(), "round-freeze-"));
      const store = new GenerationStore(directory);
      const other = new GenerationStore(directory);
      const records = new RoundRecordStore(join(directory, "rounds"));
      store.save(prior);
      let observed = false;
      const transitions: string[] = [];
      const runtime = createRoundsRuntime(
        withFakeT3Seats(
          baseDeps({
            persistGeneration: async (generation) => {
              ...
              if (observed && replaceAfterObservation) {
                other.save({ ...prior, draftingReportBoardId: "attempt-b" });
              }
            },
            loadGeneration: (id) => store.load(id),
            loadGenerationVersion: (id) => {
              const version = store.loadVersion(id);
              observed = true;
              return version;
            },
            freezeGeneration: (id, revision) => store.freeze(id, revision),
            recordRound: (id, record) => records.record(id, record),
            onGenerationTransition: (transition) => {
      ...
      const result = {
        predecessor: other.load(prior.id),
        transitions,
        record: records.read("s1")[0],
      };
      store.close();
      other.close();
      return result;
    ...
    expect(owned.record?.frozenPredecessor).toBe(prior.id);
    expect(owned.transitions).toEqual([prior.id]);
    const stolen = await run(true);
    expect(stolen.predecessor).toEqual({ ...prior, draftingReportBoardId: "attempt-b" });
