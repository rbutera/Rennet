...
describe("a seat cannot type a count (D10)", () => {
  it("no tool input on any target carries a tally field", () => {
    ...
    // figures (2026-09-05): 6 targets, 100 tools, 351 input fields between them — 94 tools
    // and 345 fields before `write_board` added one of each per target for #869s spike. They are
    ...
    expect(toolsSwept.size, "tools swept").toBe(100);
    expect(swept, "target/tool/field triples swept").toHaveLength(351);
  });
