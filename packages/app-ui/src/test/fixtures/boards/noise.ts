...
import { board, codeRef, noiseVerdict, prose, section, skip } from "./helpers";
...
export const noiseBoard: LensBoard = board(
  "noise",
  "gen1",
  "noise-gen1",
  [
  ...
  {
    skippedHunks: [
      skip(`${CHANGE}/proposal.md`, "Normative change intent — spec artifact."),
      skip(
        `${CHANGE}/design.md`,
        "The four decisions and their rejected alternatives — spec artifact.",
      ),
      skip(
        "packages/adapters/src/github-auth.ts",
        "The behavioral edit — the judgment the reviewer must weigh (Sequence/Decisions lanes).",
      ),
    ],
  },
);
