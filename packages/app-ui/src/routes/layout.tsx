...
// dragging suppresses the width transition for the LIFETIME of the drag, re-armed 200ms
// after the pointer lifts.
// ─────────────────────────────────────────────────────────────────────────────
...
export function AppLayout({ children }: { readonly children: ReactNode }) {
  ...
  // keyed on the pointer being DOWN — set on pointer-down, and on up/cancel/lost-capture
  // re-armed on a 200ms TRAILING timer, which is the behaviour INVENTORY §1 records
  // ("suppresses the width transition, which re-arms 200ms after the drag stops",
  // `components/shell.tsx`). The timer starts at pointer-UP, never on each move, so a
  // mid-drag pause can never re-arm the transition under a still-held pointer.
        ...
        <CoachDataProvider>
          ...
          session route or with the chat closed; C7 fills the dock's internals. */}
            <div
              ...
              data-open={dockOpen}
              inert={!dockOpen}
              // +4: the prototype's wrapper is `chatWidth + 4` (`components/shell.tsx`)
              // — the divider straddles the seam with a net-zero footprint, so the four
              // pixels are the gutter it needs. `effectiveChatWidth` alone runs the dock
              // 4px narrow against the prototype.
              style={{ width: dockOpen ? effectiveChatWidth + DOCK_DIVIDER_GUTTER : 0 }}
              className={cn(
                // No ground of its own — the dock shares the frame's one seamless canvas.
                "rennet-chat-dock flex-none overflow-hidden border-r border-line",
                !resizing &&
