...
// dock + 256 panel + 520 surface needs — the clamp would otherwise engage here.)
...
describe("frame chat-width clamp (520px surface minimum + ARIA range)", () => {
  it("clamps a stored width that overflows a shrunken viewport, keeping the surface floor", async () => {
    ...
    setViewport(1280); // panel 256 + surface 520 + 4 gutter leaves 500 for the dock
    const { getByTestId, getByLabelText } = mount(
      <RennetRouterApp bridge={frontDoorBridge()} history={memoryHistory("/s/review-1")} />,
    );
    await waitFor(() => expect(getByTestId("chat-dock-slot").hasAttribute("inert")).toBe(false));
    // 1280 − 256 (panel) − 520 (surface floor) − 4 (divider gutter) = 500: NOT the stored
    // 900. The rendered wrapper is that plus the gutter, so the surface keeps a full 520.
    expect(getByTestId("chat-dock-slot").style.width).toBe("504px");
  ...
  it("re-clamps when the collapsed sidebar expands and steals the dock's room", async () => {
    ...
    setViewport(1200); // collapsed 0 + surface 520 + 4 gutter leaves 676 — 500 fits
    const { getByTestId } = mount(
      <RennetRouterApp bridge={frontDoorBridge()} history={memoryHistory("/s/review-1")} />,
    );
    await waitFor(() => expect(getByTestId("chat-dock-slot").hasAttribute("inert")).toBe(false));
    expect(getByTestId("chat-dock-slot").style.width).toBe("504px"); // 500 + the gutter
    // Expanding the sidebar to the 256px panel drops the max to 420 — the dock clamps down.
    act(() => useRennetStore.getState().uiActions.setSidebarOpen(true));
    await waitFor(() => expect(getByTestId("chat-dock-slot").style.width).toBe("424px"));
  });
});
