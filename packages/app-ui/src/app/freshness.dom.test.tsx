...
describe("invalidation UX (#576)", () => {
  ...
  it("invalidates a mounted signing composition and reaches regenerated bytes without remounting", async () => {
    const stale = {
      ...reviewAt("invalid"),
      patchsets: [
        {
          id: "ps-1",
          source: "local",
          repository: { headRef: "feat/widget-refresh" },
        },
      ],
    } as Review;
    let regenerated = false;
    let regeneratedComposes = 0;
    const bridge = new MemoryBridge({
      "ask.read": () => ({
        projection: {
          stagedAsks: {},
      ...
      "review.checkFreshness": () => ({ review: stale }),
      "review.regenerate": () => {
        regenerated = true;
        return { review: { ...stale, status: "current" } };
      },
      "publish.compose": () => {
        if (regenerated && regeneratedComposes++ === 0) {
          return {
            status: "unavailable",
            reason: "The regenerated review boards are still drafting.",
            retryable: true,
          };
        }
    ...
    const history = memoryHistory("/s/rv-1?view=handoff");
    const r = mount(
      <BridgeProvider bridge={bridge}>
        <Router hook={history.hook} searchHook={history.searchHook}>
          <ReviewWorkspace review={stale} />
        </Router>
      </BridgeProvider>,
    );

    expect(await r.findByText("Patchset A pull request")).toBeTruthy();
    expect(r.getByText("Patchset A body.")).toBeTruthy();
    await r.user.click(r.getByRole("button", { name: "Regenerate" }));
    expect(await r.findByText("The regenerated review boards are still drafting.")).toBeTruthy();
    expect(r.queryByText("Patchset A pull request")).toBeNull();
    expect(r.queryByText("Patchset A body.")).toBeNull();
    expect(r.queryByRole("button", { name: /Open Pull Request/ })).toBeNull();
    expect(await r.findByText("Patchset B pull request", {}, { timeout: 2_000 })).toBeTruthy();
    expect(r.getByText("Patchset B body.")).toBeTruthy();
  });
