...
import { SuccessorAccountPanel } from "./successor-account-panel";
...
describe("SuccessorAccountPanel (#73)", () => {
  it("states each ask's status and surfaces the beyond-asks change loudly", () => {
    const { container } = mount(<SuccessorAccountPanel account={account} onAnchor={vi.fn()} />);
    const panel = container.querySelector('[data-testid="successor-account"]');
    expect(panel).not.toBeNull();
    // Two addressed, one untouched — read off the rendered statuses.
    const statuses = [...container.querySelectorAll(".successor-account-status")].map((el) =>
    ...
    const beyond = container.querySelector('[data-testid="successor-account-beyond"]');
  ...
  it("renders the LLM digest as a headline ABOVE the facts when present (#73/M25)", () => {
    const { container } = mount(
      <SuccessorAccountPanel
        account={account}
        onAnchor={vi.fn()}
        digest="Addressed two, left one, and touched a file nobody asked about."
      />,
    );
    const digest = container.querySelector('[data-testid="successor-account-digest"]');
    ...
    const asks = container.querySelector(".successor-account-asks");
    expect(
  ...
  it("MODEL-FREE FLOOR at the UI: with no digest, the facts render in full and no headline appears", () => {
    const { container } = mount(<SuccessorAccountPanel account={account} onAnchor={vi.fn()} />);
    // No digest headline…
    expect(container.querySelector('[data-testid="successor-account-digest"]')).toBeNull();
    // …but every fact is present and the beyond-asks alert still surfaces.
    expect(container.querySelectorAll(".successor-account-status")).toHaveLength(3);
    expect(container.querySelector('[data-testid="successor-account-beyond"]')).not.toBeNull();
  });

  it("anchors: activating an item navigates to that path (the moved hunk)", () => {
    const onAnchor = vi.fn();
    const { container } = mount(<SuccessorAccountPanel account={account} onAnchor={onAnchor} />);
    // Tapping the untouched ask anchors to its file…
    const items = container.querySelectorAll<HTMLButtonElement>(
      ".successor-account-asks .successor-account-item",
    );
    ...
    const beyondItem = container.querySelector<HTMLButtonElement>(".successor-account-beyond-item");
  ...
  it("renders nothing when there are no asks and nothing beyond (never an empty shell)", () => {
    const { container } = mount(
      <SuccessorAccountPanel account={{ asks: [], beyondAsks: [] }} onAnchor={vi.fn()} />,
    );
    expect(container.querySelector('[data-testid="successor-account"]')).toBeNull();
  });
});
...
describe("SuccessorAccountPanel — hunk grain + attribution (#73 wave 3)", () => {
  it("renders beyond-ask hunk rows with bucket labels", () => {
    const { container } = mount(<SuccessorAccountPanel account={hunkAccount} onAnchor={vi.fn()} />);
    const rows = container.querySelectorAll('[data-testid="successor-account-hunk"]');
  ...
  it("activating a hunk row navigates to the hunk's SPAN, not just its path", () => {
    const onAnchor = vi.fn();
    const { container } = mount(
      <SuccessorAccountPanel account={hunkAccount} onAnchor={onAnchor} />,
    );
    const askedRow = [
      ...container.querySelectorAll<HTMLButtonElement>(
        '[data-testid="successor-account-hunk"] button, button[data-testid="successor-account-hunk"]',
      ),
  ...
  it("LEGACY account (no hunk fields) renders today's path-grain view unchanged", () => {
    ...
    expect(container.querySelectorAll('[data-testid="successor-account-hunk"]')).toHaveLength(0);
