...
- **Code block card**: the reusable interface component that renders a code ref as an inline diff with file identity, old/new line numbers, additions, deletions and expandable context from the reviewed revision. A citation locates evidence; selections and annotations belong to the reviewer. It renders source from the patchset rather than copying code into board prose. Distinct from the retired "card" sense of a board element.
- **Diff view**: the raw patchset in GitHub's Files-changed shape — changed-file tree with filter, per-file cards with unified dual-gutter hunks, and viewed tracking. Not a lens: it toggles from a chip beside Map, outside the view switcher. Its line comments use the same local-comment → ask flow as code block cards, carrying the captured patchset, file, side and line range so a request-change ask retains its real diff position, including deleted lines.
...
- **Round report**: the board accounting for what a round did with the asks — addressed, partial, untouched, or beyond the asks, each item verified against the round's diff and anchored. Drafted first when a round returns (its own narrow classification prompt): it is the greeting the reviewer reads while the lens drafters regenerate, and the successor account those drafters receive.
...
- **Orchestrator harness**: the coding harness a review's session thread runs on. Chosen in the welcome, which enables that harness and writes it as the Orchestrator review role's Dual Harness override; the model council's `orchestrator-chat` job resolves that override into the harness, model and effort the thread is created with. It routes the chat thread only — the lens seats resolve from their own council rows.
