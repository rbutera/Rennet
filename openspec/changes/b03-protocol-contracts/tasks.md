...
*(Cluster-4 amendments: (a) the real `commandDefinitions` count is **62**, not 64 — reconciliation 3's arithmetic slip (69 − 7 B2 deletions = 62); all "64" below read 62. (b) The mechanical move forced splitting `index.ts`'s non-command residue into `src/wire.ts` behind the same root seam — the command schemas reference pile schemas eagerly, so `commands/` importing back from `index.ts` would be an unresolvable ESM evaluation cycle; this pulls part of task 7.1's seam reduction forward, and 7.1 now only annotates/park-comments. 14 pile schemas the command table references gained `export` (they were already public as data inside `commandDefinitions`). One adaptation in `session.ts`: `refine(isCommandName)` became `refine((v) => isCommandName(v))` — the session↔commands cycle needs call-time resolution.)*
