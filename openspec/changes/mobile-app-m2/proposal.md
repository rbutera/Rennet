...
- **Publish flow** (wireframe 23): preview (the collated outbound review, verdict + destination visible) → one-tap post (`publish.requestConsent` + `publish.review` / `publish.submitPr`) → truthful posted screen with the real URL; "Ask for changes" routes to a refine turn, never phone-editing. No sign step, no confirmation ceremony — the post button is the click.
