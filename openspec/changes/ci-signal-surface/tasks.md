...
- [x] 3.1 In `packages/core/src/forge-port.ts`, add the forge-neutral `ForgeCheckRun { name, outcome, summary, detailsUrl? }` / `ForgeCiStatus { checks, sso }` nouns and the `fetchCiStatus(ref, headOid)` method on `ForgePort`. Update every in-repo `ForgePort` fake/double to implement it (a throwing default is fine where CI is untouched).
...
- [x] 3.3 Test the honest edges: a commit with NO checks (null rollup / empty contexts) → empty `checks` (the caller's `no-checks` state, never implied-passing); an SSO `partial-results` response carries `sso` out so the caller can stamp `incomplete: true`.
...
- [x] 4.3 Wire the refinement in core: refined verdicts stamp `classifiedBy: "model"`; a model may resolve `unclassified` → `change-caused`/`environmental` but NEVER touches a deterministic verdict. Red-proof: let the model demote a deterministic `change-caused` and watch the ratchet test fire.
