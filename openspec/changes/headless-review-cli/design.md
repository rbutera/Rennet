...
### D11: Repo-precise mint in a workspace, and a loud refusal on a pre-feature daemon (amendment, #952 review)

D2/D3 give the branch arm a `branch` and a `base` but no repository, and D4's PR arm finds its row by number alone. In a single-repo project both are exact. In a WORKSPACE project holding several repos they are not: `session.mint` keys its claim on `(repository, branch)` and treats the row's `owner/name` as "what resolves the capture to the right repo" (`packages/server/src/dispatch/session.ts`, the `repository` field on the mint target, #580). A branch name is unique only within one repo, and a PR number is unique only within one repo, so both arms can capture or reattach against the wrong repository.

- **Identity the CLI carries (branch arm).** The claim's repo half must be the daemon's own canonical `owner/name`, the same string the desktop mints (`review-workspace-route.tsx` mints ``repository: `${owner}/${name}` ``). The CLI must NOT parse the origin remote and spell `owner/name` itself: a `.git` suffix or a case difference would fork the claim and mint a second session beside the one the reviewer has open. Instead the branch arm passes the resolved toplevel PATH on mint (an added optional `repositoryPath`), and the daemon resolves it to the canonical `(repository, forgeRepository)` it already derives for `locals`/`prs`. The daemon produces the identity string, so the CLI cannot drift from it. The claim key stays `(repository, branch)`; single-repo behaviour is byte-identical because a lone repo resolves to the one `owner/name` it always had.
- **PR arm scopes to a repo.** `--pr <n>` resolves through `project.detail` as in D4, then filters the rows to the standing repo: match `pr.number === n AND pr.repository === <toplevel owner/name>`. When two repos each carry PR #n and none is the standing repo, refuse and list the `owner/repo#n` candidates rather than taking `prs.find`'s first hit. An explicit `--pr owner/repo#n` form MAY be added later for reviewing a PR from outside its checkout; the path-scoped default covers the common case and needs no new grammar.
- **Pre-feature daemon fails loudly.** `base` and `repositoryPath` are additive wire fields, so a daemon predating them strips them silently (zod), reviewing against the default merge-base and, on the branch arm, with no repo identity: a wrong-repo capture with no error. Gate on the handshake's `serverInfo.features` (the additive capability record, `packages/protocol/src/session/wire.ts`), exactly as `act`/`attention` gate the phone's Stop and publish affordances. The daemon advertises a flag (e.g. `review-cli`) when it wires this seam; the CLI reads the `serverInfo` frame it already receives at connect and, when the flag is absent, refuses with the minimum daemon version named, exit 1. A named "update the daemon to vX" beats a silent review against the wrong base or repo.
- **Out of amendment scope, noted for the follow-up:** the PR reattach compares head OID; a force-push after mint should re-resolve rather than reattach to a stale OID (a separate correctness fix, not part of the identity contract).

### D11 settled contract (implemented in #952)

The two open contract values, settled and recorded here:

- **Feature-flag name: `review-cli`.** It follows the existing `serverInfo.features` naming (`attention`, `act`), reads as the capability it gates, and is advertised UNCONDITIONALLY by any daemon built with this seam (`REVIEW_CLI_FEATURE`, `packages/protocol/src/session/wire.ts`); its absence is exactly what a pre-feature daemon shows.
- **Minimum daemon version: `0.1.5`.** Read from `packages/server/package.json`, the version this seam ships in. It is named only in the human-facing refusal (`MIN_REVIEW_CLI_DAEMON_VERSION`, `cli-review.ts`); the real gate is the flag, not a version compare, so the daemon's own version is quoted beside it so the message reads truthfully.

Implementation note (smaller than the sketch above, same invariant). Rather than a new `repositoryPath` field on `session.mint`, a new read command `repository.identify { path } -> { repository, forgeRepository? }` resolves the checkout path to the daemon's canonical identity (the same `repositoryIdentity` that stamps `locals`/`prs`). The branch arm passes that identity on the EXISTING mint `repository`/`forgeRepository` fields (the desktop's own path), and the PR arm uses the same identity as the standing `owner/name` for its `pr.repository === <standing>` scoping. One read command serves both arms, no new mint field is added, and the daemon still produces the identity string so the CLI cannot drift, which is the invariant the sketch's `repositoryPath` existed to protect. The command sits behind the `review-cli` flag, so a pre-feature daemon is refused at connect before it is ever reached.
