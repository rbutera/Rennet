...
**Sidecar, private base directory.** The daemon builds the vendored server with `vp pack` and spawns `node <bundle> serve --host 127.0.0.1 --port 0 --no-browser --base-dir <dataDir>/t3 --bootstrap-fd 3`, token piped into fd 3, `T3CODE_TELEMETRY_ENABLED=false`, and every `T3CODE_RELAY_*` and `T3CODE_CLERK_*` key stripped. The base directory is Rennet's, never `~/.t3`: Rai does not want the user's own T3 install touched. Provider setup and secrets from a standalone install are irrelevant because T3's defaults use the user's normal login, and the only thing Rennet must supply is absolute binary paths.
