...
**Repair is a pointer prompt.** `renderRetryPrompt` splits into `renderRepairTurn(draft, pointers, frozenIds)`: the thread already holds the base prompt and the draft, so the repair turn carries only the lint pointers, the frozen ids and the instruction to emit the corrected board. The base prompt is byte-for-byte what it is today, moved to `@rennet/prompts` as the repair partial's sibling; `tasks.md` includes the token measurement (before: base + draft + pointers re-sent cold; after: pointers only) in the PR description per AGENTS.md. Structured output travels once per turn as the schema; the Claude structured-output ledger patch continues to carry it.

**Usage from T3, not from the SDK.** The T3 leg reads the settled turn's usage and duration from the thread projection (the turn record T3 keeps per turn) and records one `TurnMetric` per turn through the existing collector, labelled `board.<jobId>`. Codex turns under T3 report tokens with no dollar figure, as today.
