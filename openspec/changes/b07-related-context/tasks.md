...
- [ ] 3.3 Tests: full flow over canned gh runner + stub runTurn (dossier items carry provenance + fetched-at; every-item-bounded; missingConfig fact on an unconfigured JIRA ref; failed ref fetch yields a typed failure item-absence, not a crash); store round-trip (write → fresh read → identical bytes via serializeDossier).
- [ ] 3.4 Gate green.

## Cluster 4 — project scout

- [ ] 4.1 Settings rows (reconciliation 3): `SETTINGS_REGISTRY` gains the issue-tracker section (tracker kind, project key/prefix, JIRA/Linear base URL, token env var NAME) + the §4 non-tracker facts the ladder does not already resolve (worktree base-dir convention, gate command — implementer verifies which exist and records it). Layers per declaration: detected+global+repo where scout can detect, global+repo otherwise.
...
- [ ] 4.3 Wire at project add (re-runnable): the existing project-add path in server triggers the scout (implementer inspects where processing starts and records the point, B04 precedent). Missing-config asks surface as the typed facts (reconciliation 7).
