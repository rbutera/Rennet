...
- [x] 5.1 Retrieval at review-session start (reconciliation 8): wire the seam at the existing choke point with the same deps pattern as the swarm runner; record the actual point in the ledger. Per-round re-run is B8's — leave the seam documented, not wired. DONE (re-pointed in the round-1 fix loop, amendment 19): the point is dispatch's `review.capture` / `review.openPr` via `DispatchDeps.onReviewOpened` — `createLiveCanvasOpsBackend` (amendment 10's original answer) turned out to be symbol-lookup-only in production; dossier persisted keyed `target@patchsetId` unchanged.
