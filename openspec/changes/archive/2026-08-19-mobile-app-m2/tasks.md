...
- [x] 7.3 Codex review hardening (findings 1–11, all accepted; finding 3 re-shaped). Landed: full publish preview (1); bound compose artifact refusing cross-review/stale posts (2); shade answers bound to the attention id, consumed atomically for exactly-once (3); replay-dedup + truthful failed-answer notification (4); reattach real in-flight + monotonic ask-stream seq dedup (5); exactly-one terminal event + persisted interrupted replacement (6); duplicate-live-id rejection so Stop-on-B never aborts A (7); repo-relative path validation at ingestion + compose + projected-error redaction (8); exact-then-unique kickoff repo matching (9); exact-id attention acks + identical-refresh suppression (10); refined/bounded ask-only action schema parsed before use (11). See design.md "Codex review hardening". Both dependency decisions admitted by the lead and wired, vetted per the Dependency Standard: finding 4's background-terminated action handling via `expo-task-manager` (`~55.0.18`, first-party, SDK-matched — module-scope `TaskManager` task answering an ask headless on Android) and finding 9's `Intent.EXTRA_TEXT` read via `expo-share-intent` (`6.1.1`, MIT, SDK-55 line, Android-only config plugin; iOS share extension recorded as the follow-up). Recorded in the dependency-standard doc.
