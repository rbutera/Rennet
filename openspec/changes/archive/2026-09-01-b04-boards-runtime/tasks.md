...
- [x] 3.2 Grep proof of the invariant recorded in the test or a comment: no other workspace module calls `BoardService.apply` (or constructs board ops) — `sh -c 'grep -rn "\.apply(" packages/ apps/ --include="*.ts" | grep -v test'` reviewed; the only board-op writer is whiteboard-client. *(Review-fix amendment 9d: the recorded grep became an executable test in `whiteboard-client.test.ts` — it re-runs the scan at test time against an allowlist holding only whiteboard-client.ts, so a second writer fails the suite instead of aging a comment.)*
