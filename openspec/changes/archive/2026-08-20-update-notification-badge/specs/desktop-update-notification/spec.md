...
Keep a running desktop app current: poll for new releases frequently, signal a ready update through a badge on the Rennet logo in the app chrome, and apply the update only when the user chooses to restart into it.
...
- **THEN** the logo renders exactly as before, with no badge and no interactive affordance beyond what exists today

### Requirement: User-invoked apply

Clicking the badged logo SHALL present a prompt offering to restart into the new version or dismiss. Choosing to apply SHALL restart the app into the downloaded update. Dismissing SHALL leave the badge in place and never re-prompt on its own. The update SHALL never apply without the user choosing it.
...
On hosts where the updater cannot run — unsigned macOS builds pending code signing, Linux, development runs, and the daemon-served browser shell — the feature SHALL be a silent no-op: no badge, no prompt, no error surfaced to the user.
