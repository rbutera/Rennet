...
- **A deterministic classifier with an honest floor.** A pure, $0, model-free function in `core` classifies each failing check: real changed-path overlap wins; otherwise only infrastructure-contextualised machinery signatures may yield `environmental`; everything else is `unclassified` — surfaced, never guessed away. Bare application error codes and domain-oriented check names are not infrastructure proof. A deterministic `change-caused` can never be demoted by any later stage.
- **An optional light-model refinement seat.** A new Model Council job (`ci-failure-classification`, light tier — resolving to the Codex light seat when installed, per Table 1) may refine ONLY `unclassified` failures to `change-caused`, or leave them `unclassified`, in one batched turn. It can never mint `environmental`; only deterministic infrastructure signatures can. It draws from the SAME shared review invocation budget and never refuses the review.
- **Change-caused failures become findings when placeable.** Each `change-caused` failure is folded into the Flagged lens as an additive `FindingElement` when an offered hunk resolves. Placement is carried explicitly; an unplaced failure remains fully visible in the panel instead of acquiring a hallucinated anchor or disappearing.
...
- **Never blocks, structurally.** The fetch and optional refinement have separate deadlines and abort signals. A thrown or hung fetch resolves to `unavailable`; a thrown or hung refinement leaves deterministic verdicts visible. Incomplete, truncated, empty, or neutral-only reads never become passing. No dispatch handler, sign affordance, or publish path reads CI state.
