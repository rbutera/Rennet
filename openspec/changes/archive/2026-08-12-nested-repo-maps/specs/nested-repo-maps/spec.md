...
Deterministic, by-reference composition of one-repository Repo Maps across tool-declared monorepo scopes, gitlink-pinned submodules, and app-owned multi-repo `WorkspaceContext`s. Builds on `build-repo-map-lifecycle` without redefining its `ProjectSnapshot`, storage, or base/overlay model.
