...
- **`CodexAdapter implements HarnessPort`** in `@rennet/adapters` — the second harness slot, peer of `ClaudeAdapter`. Injected transport seam (mirroring `ClaudeQueryFn`): the composition root spawns the discovered `codex` binary (`discoverCodex`) with `codex exec --json` in the session's cwd, full capability, no approval plumbing, no read-only posture. JSONL frames decode the official `item.type` discriminator (`item.item_type` remains a compatibility fallback) into the existing `HarnessEvent` protocol: item starts become `tool.started`, completed command/MCP items become `tool.output`, raw native frames are retained, and unmodelled frames pass through. Token usage is normalized into disjoint input/cache-read/cache-write/output fields. `AbortSignal`/`interrupt()` terminates the process tree and session close waits for transport completion. Never reads a credential (Codex `auth.json` included). Host locus only; WSL codex is #334's seam.
- **Desktop composition** resolves the existing `orchestrator-chat` seat through the selected harness. A Codex-selected seat reaches the injected `CodexTurnTransport` plus the external canvasOps transport; no additional consumer is moved to Codex by this change.
- **Generalised conformance suite** (`@rennet/core`, pure over `HarnessPort`): one suite run against every adapter; each passing check flips exactly one capability layer from `false` via the existing `buildCapabilities` evidence mechanism — flags are exactly the passing set, never declared. A deliberately broken port refutes every individual check; if any check cannot be made to fail, the suite refuses to certify. The interrupt probe drains first, waits for readiness, calls only `session.interrupt()`, and requires cancellation plus transport completion. Token usage does not certify context-window capacity. The default gate runs it hermetically against fake transports (zero spend); a gated `.real.test.ts` runs it against the real binaries.
- **`testedRange` derived, never hand-edited** (backlog bead 63): real conformance runs record the binary versions they passed against into a committed per-harness artifact only when the complete expected pass/fail capability matrix matches; adapter descriptors read `testedRange` from that artifact. `ClaudeAdapter`'s explicitly permitted migration seed remains, while Codex starts absent until its first genuine matching real run.
