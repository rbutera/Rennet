...
- [x] 2.1 `packages/server/src/projection.ts` per design D2/D3: outbound `projectFrame` (structural table from the design Context list) + `scrubRoots` free-text substitution; inbound `resolveInputs` (repoKey → host path via project store; unresolvable → rpcError invalid_input).
- [x] 2.2 The path-field table is generator-checked: a command/frame field typed as a host path that lacks a table entry fails a test loudly (allowlist maintained beside the table).
- [x] 2.3 Unit tests: every listed outbound field projected (fixture review/patchset/project shapes); inbound resolution round-trip; free-text root substitution; unresolvable-ref error.
...
- [x] 6.3 `NX_DAEMON=false pnpm check` green; Playwright e2e untouched, failure set per baseline.
