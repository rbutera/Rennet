...
- [x] 7.1 In `chat/chat-data.ts` ONLY: replace the `SessionTranscriptProjection` context read with `useCommand("session.transcript", { reviewId })` (B9's projection) and resolve the live session's `reviewId` for the `onAskStream` subscription from the real route/session (today the tests inject it). Delete the stub context. **Exact one-file swap (as built):** inside `useChatDock()`, the line `const projection = useSessionTranscript();` becomes a `useCommand("session.transcript", { reviewId })` read whose result supplies `rows` / `trail` / `contextWindow`, and `const reviewId = projection.reviewId;` is replaced by resolving the slug from the route (`useRoute(ROUTES.session)` → the session's reviewId, the same swap `routes/slug.ts` makes). Then DELETE the context block (`SessionTranscriptContext` / `SessionTranscriptProvider` / `useSessionTranscript` / `EMPTY_TRANSCRIPT` and the `SessionTranscriptProjection` interface's use as a context) and drop its barrel re-exports in `chat/index.ts` + `app-ui/src/index.ts`. Nothing else in the dock changes — every component already reads the resolved `ChatDockModel`. Tests move from `SessionTranscriptProvider` to a `session.transcript` MemoryBridge handler. [archive 2026-09-01: chat is a durable live action surface on main (#620)]
- [x] 7.2 Compaction meter binds to B9's real harness-reported context figure + `compact_boundary` events (still honest "unknown" when the harness reports none). [archive 2026-09-01: chat is a durable live action surface on main (#620)]
- [x] 7.3 Live E2E in the real app: a real orchestrator turn streams into the dock; a real compaction boundary renders its row. Deferred with cluster 7. [archive 2026-09-01: chat is a durable live action surface on main (#620)]
