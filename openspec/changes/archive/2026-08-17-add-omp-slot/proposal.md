...
- **Discovery and Bun-aware health**: `discoverOmp` reuses the shared discovery machinery (login-shell PATH harvest + curated locations including `~/.bun/bin`, execute-to-prove, `RENNET_OMP_BIN` override). omp is Bun-first (`engines.bun >= 1.3.14`, TS entry point): discovery resolves Bun before probing omp, enforces `>=1.3.14`, probes and launches the script through that exact runtime, demotes asdf omp shims behind real installs, and consumes locus `PATHEXT` on Windows. A discovered `omp` with no runnable Bun reports health honestly with a reason naming Bun instead of crashing at first spawn.
...
- **Conformance, hermetic by default**: the existing suite runs against an omp-shaped fake transport in the default gate (zero spawns, zero spend); a gated `.real` test (`RENNET_LIVE_OMP=1`) runs it against the installed binary and records the tested range only after the full expected matrix matches. Hermetic evidence earns only `implementedByAdapter`; `structuredOutput`, usage, and cost stay absent because omp RPC accepts no output schema and the transport does not request stats.
- **Supported, bounded real transport**: canvasOps is declared through a scratch extension's `mcp.json` using omp's `{ type: "http", url }` schema and `--extension`, never the unrelated `--config` overlay. RPC stdout frames and stderr are byte-bounded; malformed, oversized, rejected, construction-failed, and iteration-failed turns produce one failed terminal outcome instead of disappearing or hanging.
...
- `packages/adapters`: new `omp-adapter.ts` (+tests), new `omp-turn-transport.ts` composition root (+tests), `discoverOmp` in `harness-discovery.ts` (+tests), gated `omp-conformance.real.test.ts`; `harness-tested-range.json` remains untouched until a full-matrix real run.
- `packages/core`: `HarnessDescriptor.testedRange` is absent until earned and health can report `untested`; `harness.ts` already carries `"omp"` in `HarnessId`. A conformance fake for the omp shape lives with the existing fakes in tests.
