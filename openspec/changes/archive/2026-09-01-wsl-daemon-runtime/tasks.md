...
- [ ] 5.1 Full `ASDF_NODEJS_VERSION=24.16.0 pnpm check` green (format, architecture, licenses, lint, typecheck, test, build). Host-locus behavior regression tests pass unchanged. [archive 2026-09-01: lancelot verification not recorded here; WSL follow-ups owned by open #479]

## 6. Field proof (lancelot, manual — deferred)

- [ ] 6.1 On lancelot: open a WSL-locus project; confirm the daemon runs inside the distro (native watcher, 0 EISDIR in `daemon.log`), GitHub egress works from the distro, and the shell routes to the distro daemon's port. Confirm a host-locus project is unaffected. Record the result before archiving. [archive 2026-09-01: lancelot verification not recorded here; WSL follow-ups owned by open #479]
