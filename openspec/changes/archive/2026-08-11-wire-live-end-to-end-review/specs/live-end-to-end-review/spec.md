# live-end-to-end-review

The wire-up that turns the fixture-backed `canvasOps@2` surface into a live product: a production `CanvasOpsBackend` composed over real review state, an orchestrator session served that surface through a live harness, the Repo-Map serving real snapshot-derived data, and the model-enriched canvases actually rendering on a real harness. Point Rennet at a real change and the lenses AND the repo map are live, end to end, in the desktop app.
