# knowledge-disposition Specification

## Purpose
Defines how a reviewer confirms or rejects model-authored knowledge statements and how those dispositions persist without changing the statement or its evidence.
## Requirements
### Requirement: Statement disposition persists
The system SHALL expose a `project.knowledgeDisposition` command that sets a knowledge statement's status to `confirmed` or `rejected` by id and persists the updated set through the local knowledge store. It SHALL preserve every statement's id, claim, evidence, and deterministic ordering.
...
- **THEN** the stored statement carries status `rejected`, remains in the set, and is excluded from hypothesis and confirmed presentation and from ask context
...
The Context Map surface SHALL offer confirm and reject actions on each hypothesis statement and a discuss action that feeds the statement into the conversation rail. Disposition actions MUST render the persisted result returned by the daemon.
