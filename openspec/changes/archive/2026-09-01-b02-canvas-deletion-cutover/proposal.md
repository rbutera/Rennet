...
- `ui-verification.ts`: DELETE → **KEEP (trim)**. Its deterministic `classifyUiSurface` / `UI_SURFACE_CLASSIFIER_VERSION` / `RunUiVerificationResult` are read LIVE by `create-server`'s flagged path (the $0 immediate UI-surface status); only the model verify-ui turn (`runUiVerification`) is deleted. `ui-verification.test.ts`: DELETE → **KEEP (trim)** (B2 round-2 review, PR #496): the deterministic `classifyUiSurface` / `isUiSurfacePath` assertions are restored; only the model-turn (`runUiVerification`) tests stay deleted.
- `angle-generation.ts`: DELETE stands, but its deterministic `buildOfferedManifest` (read live by `create-server`'s flagged finding path + noise) is **rehomed to a new `offered-manifest.ts`** rather than lost; a new `offered-manifest.test.ts` (B2 round-2 review, PR #496) carries the `buildOfferedManifest` assertions from the deleted `angle-generation.test.ts`.
...
- `element-diffs.ts`: the table said "inline `compareCodeUnits`"; the actual dying-`canvas.ts` dependency is `AdmittedDocument` + `isProposalBody` (element-diffs uses a local `compare`, never `compareCodeUnits`). Those two were inlined. `element-diffs.test.ts` stays KEEP (trim): its REAL SHAPE (#250) case (geometry-complete occurrence + sole-ownership assertions, #250 r2 F3) — dropped in cluster 4 because it drove the deleted `buildCanvas` producer — was **ported to the file's local-shape fixture pattern** (B2 round-2 review, PR #496); the buildElementDiffs assertions are carried as-is, only the `buildCanvas` plumbing replaced with the hand-built `setWith`/admitted-docs shape.
...
**B2 amendment (cluster 6):** the `BASE_CONTRACTS` registry is also dropped, not carried — it is a `Record` keyed on five dead-pass contracts (finding/decision/ordering/rollup-narration/hypothesis) plus two survivors, so it cannot be absorbed without resurrecting the dead contracts, and it has zero surviving consumers (verified by grep). The three genuinely-consumer-free survivor consts that reference NO dead pass (`PROMPT_CONTRACT_VERSION`, `LOGICAL_ORDERING_TERMS`, `FORBIDDEN_ORDERING_TERMS`) and the two unconsumed-but-named decomposition contracts ARE carried verbatim (deleting nothing beyond the package + the named dead-pass contracts). The instruction survivors land in `packages/prompts/src/prompt-contracts.ts` (`git mv` from the old `index.ts`, history preserved) and are re-exported from `packages/prompts/src/index.ts`; the six instruction test files are deleted with the package.
