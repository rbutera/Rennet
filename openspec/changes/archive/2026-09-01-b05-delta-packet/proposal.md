...
5. **DeltaPacket type home = core/delta, not protocol.** It is the drafter *input assembly* (engine asset places the seam in core); its constituent shapes (KnowledgeSet, DossierItem, SuccessorAccount, HunkId) are the protocol contracts. B8 serializes the packet into prompts; if a wire contract emerges then, promoting it is B8's call.
...
15. **Docs/ledger aligned with the real packet.** `context-assembly.md` claimed the packet contains "the element differ's output" (it does not — the differ feeds lineage/successor-account, B8) and that ownership facts appear as not-assessed marks (per amendment 7 they are honestly ABSENT; fan-in is the not-assessed mark). Reworded to the real field list. Ledger item 5 dropped its `CodeRef` constituent claim — the packet carries no CodeRef.
