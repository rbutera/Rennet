...
- [x] 2.2 Attention planner: six-event taxonomy (closed, sourced from the protocol enum), each with substance + deep-link route (`deepLinkFor`); per-client presence tracking in the listener; connected-and-focused → live event only, others → push; high-priority families always reach every client; silent (processing-finished) posts no push; unit tests per family and per presence case in `attention-planner.test.ts`. **Source wiring (updated, #383 review batch):** `review-finished` (capture/openPr/regenerate), `ask-pending`, and `turn-failed` are now wired to their REAL sources — ask-pending/turn-failed to the streaming `review.ask` turn lifecycle (raise on pending, clear on settle, raise turn-failed on error/interrupt). `handoff-completed` + `publish-ready` stay seam-only until M2 (their flows are M2 scope); `processing-finished` is silent (no push). `running` on the projected summary is sourced from a real in-flight-review registry (marked by review-scoped turns), NOT `pendingPatchsetId` (staleness). See protocol-compatibility for the live-in-M1 family list.
...
- [x] 3.1 Scaffolded Expo + expo-router app; Metro resolves the workspace packages (metro.config.js watchFolders + nodeModulesPaths). **Inferred-vs-explicit Nx targets:** the `@nx/expo:application` generator's inference pulls prettier, per-app eslint, jest, and nx.json plugin config that conflict with the workspace's biome/eslint/vitest gate, so `apps/mobile` uses **explicit** `lint`/`typecheck`/`test` targets (plain commands, the workspace's own tools) exactly as every package does. No `build` target (Expo export is native distribution, M3). **(#383 review batch, finding 17):** because the app uses explicit targets, `@nx/expo` itself went unused and was **removed** — and removing it also removed the only package (`@expo/require-utils`, its transitive dep) that peer-wanted TS `^5`, so the workspace-wide `peerDependencyRules.allowedVersions: typescript` override was **dropped** too (install is clean without it). Dependency-standard "Mobile stack" section updated.
