...
- **The desktop scale becomes documented truth, then the CSS converges on it.** DESIGN.md gains an explicit desktop type ramp (10/11/12/13/14/16/19/22px + the front-door display clamp) and radius scale (4/6/8/12/16px + pill/circle geometry), derived from the wireframe kit and the audited live values. `.impeccable/design.json` regenerates from it so the design-lint learns the ramp. Every off-ramp literal in `styles.css` and `canvas.css` (all fractional sizes, 9px type, the 5/7/9px radius cluster, 18/20px headings) collapses onto its ramp neighbour. No blanket suppression; the per-value split is recorded in design.md. Evidence: the impeccable design-lint reports **zero** radius/font-size findings.
...
- **The deferred visual decisions land:** conversation threads align in-rail to their anchor line when the row is visible (structural no-reflow guarantee preserved; honest stacked fallback when it is not) — the one behavior-contract change in this pass; and `RennetMark` swaps its placeholder mono "R" for the real split-disc glyph from `site/brand/rennet-glyph.svg`, token-driven (`--mark-ink`/`--private`).
...
- `canvas-ui`: one added requirement — conversation threads visually align to their anchor line in the right rail when the anchor row is rendered, with an honest stacked fallback and the existing no-reflow guarantee intact. Everything else in this change is presentation and changes no behavior contract.
...
- **`packages/ui/src/components`** — `icons.tsx` (`RennetMark` glyph), `conversation-host.tsx`/`conversation-cluster.tsx` (anchor-aligned positioning via the existing `data-anchor-key` hooks), small class/DOM adjustments where a surface's structure needs them (each DOM-tested).
- **`DESIGN.md` + `.impeccable/design.json`** — the desktop scale, enumerated; **`docs/`** — delivery-order closing entry.
