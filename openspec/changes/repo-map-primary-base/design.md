...
The tier is **gated on `origin/HEAD` existing** (`symbolic-ref --quiet refs/remotes/origin/HEAD`, existence only — a dangling TARGET still exits 0, so the fallthrough above keeps working), because the resolver's caller-less probe does not require one: ungated, the tier would answer `main`/`master` in a clone that has no `origin/HEAD` at all, and `baseRefResolution` is provenance stamped on the manifest — a `symbolic-head` that read no symbolic head is a lie. The gate keeps the tier's reach where it is for a clone with no `origin/HEAD` at all, so a remoteless clone still fails closed to the caller's own fallback rather than newly resolving. With a dangling target the tier now answers where it used to fall through to `configured-upstream`, which is the fallthrough this decision wants, and the label then means "the tier that consulted `origin/HEAD`", not "`origin/HEAD` named this ref".

The explicit tier applies the newest-spelling rule to whatever branch name the caller supplies, not only to `main`: project discovery records the current branch as `primaryBranch` when a clone has no `origin/HEAD`, so a feature branch can reach this tier, and it then maps at the newer of its two spellings, with the remote-tracking one winning on divergence. That is the same answer capture gives for the same name, which is the point.
