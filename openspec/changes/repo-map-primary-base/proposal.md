...
- `baseRefResolution` provenance keeps its three values; `explicit-setting` still means "the caller named it", `symbolic-head` still means "`origin/HEAD` named it". Which spelling won is visible in `baseRef`.
