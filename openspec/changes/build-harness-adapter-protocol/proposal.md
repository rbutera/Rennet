...
> ⛔ **SUPERSEDED 2026-08-11 by RULE ZERO (CLAUDE.md).** No consent gates, no gates, no robustness for robustness' sake. "Defaults to a read-only tool posture" is withdrawn — the adapter imposes no tool posture and inherits the user's own harness permissions, because a capable agent that can write and push is the product. Every other clause in this bullet stands.
...
> ⛔ **SUPERSEDED 2026-08-11 by RULE ZERO (CLAUDE.md).** No consent gates, no gates, no robustness for robustness' sake. Strike "and enforces a read-only review posture" from this capability — the adapter enforces no posture; it drives a fully capable harness session.
