...
- **The verdict informs, never gates (Rule Zero).** The verdict rides an additive-optional `adjudication` field on the disagree row. The row always renders; an adjudicated flare shows which side the code supports. A failed, capped, or budget-exhausted turn is honest `insufficient` with its reason — never a drop, never a fabricated verdict. Rendering never waits on calibration.
...
- `packages/core`: new `finding-adjudication.ts` (pure pass + prompt assembly via `@rennet/instructions`), new `adjudication-corpus.ts` (synthetic fixtures) and pure scorer; pipeline wires the pass after reconcile, budget-shared and capped.
