...
- Runs after verification in the live desktop flagged flow: generate → reconcile → verify → deliver verified rows → **adjudicate concurrently as post-hoc enrichment**. A row verification already refuted is gone before adjudication and spends nothing. Dispatch stores the background result under review + patchset + review mode; the renderer reads that key after the initial response and updates the already-visible rows when it completes. The canvas pipeline's unused `adjudicationConfig` mirror is deliberately absent.
...
`packages/adapters/src/adjudication-calibration.real.test.ts` (env-gated like the other `.real` suites) materialises every corpus item into its own temporary git repository, runs both installed harnesses' finding seats there under one production-sized shared budget, reconciles, and adjudicates only the finding joined to the fixture's seeded claim and anchor. Missing, duplicate, unknown, failed, or ambiguous outcomes abort before recording. The recorder is absent from the public adapter barrel, requires an explicit scratch target for ordinary calls, permits the committed target only under the live env flag, and replaces `adjudication-calibration.json` by atomic rename. Nothing consumes the table as a gate.
