...
- [x] 5.4 Contract tests for the real path (reconciliation 5): assignment routing per scenario reaches the right harness port with the right model/effort; turn construction carries the worker output schema; no live model call anywhere in the gate. (Landed: adapters/knowledge-swarm.test.ts — both-scenario luna/low workers on the codex executor + sonnet-5 verify on the Claude port with schema identity asserted; claude-only haiku/sonnet split; resolved-but-missing harness refuses honestly. Deps loosened to `Pick<>` seams so the fakes typecheck without casts.)
- [x] 5.5 Gate green (positive proof: `pnpm nx run-many -t typecheck -p rennet-core rennet-server rennet-adapters` — no consumer lost an export). (Both green: proof "Successfully ran target typecheck for 3 projects"; gate "Successfully ran targets lint, typecheck, test for 10 projects and 9 tasks they depend on", GATE_EXIT=0 on its own line.)
