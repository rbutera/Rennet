...
- **The round report as greeting + progressive reveal** — after the host verifies the persisted report read-back, the report board fills the surface while regeneration is still nonterminal. A cold board-route reload reconstructs it from durable state rather than an in-memory arm. A later failure returns to the run failure view. **View the New Boards** appears only at terminal composition, derived from state and never a disabled button waiting to enable.
