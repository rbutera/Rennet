...
2. **Residency**: `window-all-closed` no longer quits — it flips residency (macOS `app.dock.hide()`; restore via `app.dock.show()` before window recreate). `activate` (macOS) and tray "Open Rennet" share one `ensureWindow()` (focus if exists, else recreate through the existing window-creation path). Real quit uses an internal `isQuitting` flag so `before-quit`-era close handlers don't fight the tray close-to-hide.
3. **Owned-daemon stop**: a `stopOwnedDaemon(dataDir)` helper in the supervisor module mirroring `rennet stop` exactly (read claim → SIGTERM → bounded poll for claim-gone → truthful warning on timeout). The quit action: stop owned daemon (if any) → `app.quit()` with `isQuitting` set. Never touches remote connections. Label derivation: claim file present + healthy ⇒ "Quit Rennet and stop daemon".
