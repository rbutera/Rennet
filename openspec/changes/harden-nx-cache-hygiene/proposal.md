...
- Tighten `licenses` inputs (defense-in-depth, found in second review): add `sharedGlobals` so a resolution-config change (`pnpm-workspace.yaml`, `.npmrc`) invalidates it. This was under-covered but not exploitable — any license-relevant change also regenerates the already-hashed `pnpm-lock.yaml` — so it is completeness, not a live stale-pass.
