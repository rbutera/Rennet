...
- [ ] 1.2 `packages/server/src/dispatch/index.ts` (or `dispatch.ts` shrunk to the composer): build a `Map<commandId, handler>` bound from `protocol/commands`, assembled from the per-family modules. Preserve the `createDispatch({...})` DI surface and its consumers in `create-server.ts` — same deps in, same dispatch-call shape out.
- [ ] 1.3 Delete the old `switch (name)` body. The map is the only router. Wire the map lookup so an unknown/unregistered id fails the same way the switch's default did (no new gate — Rule Zero).
