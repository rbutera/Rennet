...
- [x] 1.2 **Structural rule task:** the active page is read from the **route param** (`/settings/:page`) and the Projects scope from **`?project`** — no `useState` page switch anywhere in `settings/`. Each page is its own module (`settings/environments/`, `settings/appearance.tsx`, `settings/shortcuts.tsx`, `settings/projects/`). Wire `routes/app.tsx` to mount `settings/settings-screen.tsx` at `ROUTES.settings`, deleting the interim inline wrapper. *(reconciliations 3, 4; autopsy S2)* — **Done:** `parseSettingsPage(:page)` drives the page (no `useState`), `routes/app.tsx` mounts the real screen + `PriorSurfaceTracker` (interim inline `SettingsScreen` deleted). `appearance.tsx` is its own module; Environments/Keyboard-Shortcuts/Projects render an honest `SettingsPagePending` interim until their clusters land each as its own module. `?project` scope is read by the Projects page (cluster 8). **Slug reconciliation:** the shipped C3 sidebar navigates to `settingsPath("keybindings")`; the packet deep-links `/settings/shortcuts`; the spike used `machine`. `pages.tsx` makes `keybindings`/`environments` canonical and aliases `shortcuts`→keybindings, `machine`→environments, so every entry point resolves.
- [x] 1.3 `settings/provenance-chip.tsx`: the shared chip naming the resolved rung (builtin/detected/global/repo), reused by every layered value. `settings/backing-file.tsx`: the shared monospace caption naming a section's backing file (`client-settings.json`, `daemon-settings.json`, or `.rennet/`). *(claims 578, 579)* — `ProvenanceChip` takes the live `ResolvedProvenance` (salvaged from the deleted settings-screen's `Provenance`); `BackingFile` is the mono caption `Section` renders.
- [x] 1.4 DOM test over `MemoryBridge`: the shell mounts, the `esc` hint shows, Escape leaves to the prior surface, and a mounted chat/board fixture survives the visit (assert not remounted). Cluster gate green. Commit. — `settings/settings-screen.dom.test.tsx` drives `RennetRouterApp` over memory history: mount from the route param, the four pages + `esc` hint, cold deep-link selects the page (structural proof), Escape + the back arrow leave to the prior surface, and the `chat-dock-slot` node is identical across the round-trip.

## 2. The settings command seam (the single resolution point; `settings-data.ts` dies)

- [x] 2.1 `packages/app-ui/src/settings/data/` — the one seam every value reads/writes through (`useCommand`/`useMutation` over `settings.*`). Bind the existing commands live (`settings.get`, `setAppearance`, `setKeybinding`, `setRepoVisibility`, `setRepoLocus`, `pinRepoValue`, `resetRepoValue`, `guidance`). Define the B10-absent projections (environments/hosts, source-control detection, agents detection, model mappings, project glyphs, worktree pattern, issue tracker) as typed reads through this seam, carrying the `{ value, layer }` provenance shape as the contract. *(reconciliations 5, 8; Objective "rewrite every value onto settings.* commands")* — **Done:** `data/live.ts` binds all eight live commands through the data seam (intention-named hooks, uniform `settings.get` invalidation, never `bridge.invoke`); `data/projections.tsx` is the `SettingsProjection` context (mirrors C3's `sidebar-data.ts` EMPTY_PROJECTION) carrying every B10-absent read + its edit; `data/provenance.ts` is the `Layered<T>` keep contract + `toProvenance` lift into the shared chip. Appearance migrated onto `useSettingsView`/`useSetAppearance`.
- [x] 2.2 Add `settings.*` handlers to `packages/app-ui/src/test/memory-bridge.ts` (and/or per-test fixtures) so every page is testable now — provenance chips render and edits persist **to the bridge**, never a hollow pass. The absent-projection reads surface an honest empty/deferred state, not a thrown render error (mirroring C4's citations seam). — **Done:** `test/fixtures/settings.ts` is the stateful `SettingsStore` → MemoryBridge handler set (`settings.get`/`guidance`/`setAppearance`/`setKeybinding` persist; malformed refuses per Rule 75). The projection reads honest-empty by default (`EMPTY_SETTINGS_PROJECTION`), stateful when a test supplies the provider. Proven in `data/settings-data.dom.test.tsx` (scheme write persists + rung moves builtin→global; projection empty→supplied).
- [x] 2.3 Delete `spikes/`-shaped fixture dependence: confirm nothing under `settings/` imports the spike `lib/settings-data.ts` (it has no successor — reconciliation 8). Port only `lib/os-glyphs.ts`, `lib/theme-packs.ts`, `lib/code-theme.ts`, `project-icon.tsx` into `app-ui` (real libs, not fixtures) as their consuming pages need them. Cluster gate green. Commit. — **Done:** grep `settings-data` / `spikes` over `settings/` returns only comments, no imports (fence.test.ts guards `src/test` reaches generically). All four assets ported into `settings/assets/` (os-glyphs.tsx, project-icon.tsx, theme-packs.ts, code-theme.ts) — theme-packs/code-theme rebound to the real `@rennet/theme` pack + code-theme ids (`data-rn-theme`/`data-rn-code-theme`), not the spike's shiki list.

## 3. Environments page — host cards + daemon

- [x] 3.1 `settings/environments/environments-page.tsx`: page titled "Environments"; "This Machine" as the local card (never removable); remote hosts as cards. Section header carries its own **Add Environment** button (second entry point beside the sidebar's). *(claims 583, 584)* — Add Environment shares the sidebar's `openDialog("add-environment")` flow; the always-present local card is synthesized from the bridge (`platform`→os, `version`→daemon) when the projection is empty (live client), else the projection owns the hosts.
- [x] 3.2 `settings/environments/host-card.tsx`: one card per environment; header shows the OS glyph (macOS/Linux/Windows; WSL = Windows mark + mono `WSL` chip) via ported `os-glyphs.ts`, the name, and either the mono address or a "Local" chip. *(claims 585, 586)* — OSGlyph is the ported real lib (`settings/assets/os-glyphs.tsx`).
- [x] 3.3 Inline rename: Enter commits, Escape cancels, an emptied name keeps the old one; a rename flows through to the sidebar host-group header (one hosts state — write through the seam/store, not a local copy). *(claims 587, 588)* — rename writes `projection.renameHost` (Escape stops propagation so settings stays open); the DOM test proves the write reaches a second reader of the SAME hosts state.
- [x] 3.4 Remove offered on remote hosts only (never the local machine); Remove opens the one sanctioned destructive confirmation naming the project + session counts it takes and stating the machine itself is untouched. *(claims 589, 590)* — `SettingsHost.projectCount`/`sessionCount` feed `removeDescription`; local card renders no Remove control.
- [x] 3.5 Daemon meta line: version when reachable; "Not connected — daemon unreachable, version unknown" when nothing can be asked; "Not connected — last seen running Rennet daemon v<n>" for a previously-seen unreachable host (never invents current state). Reconnect appears only when unreachable; Update Daemon appears only when that host has an update (button-only, reconciliation 6). *(claims 591, 592, 593, 594)* — `daemonLine` derives all three from `DaemonInfo`; Reconnect/Update Daemon are button-only affordances (their daemon dial/apply is B10-side detection, cluster 10), their VISIBILITY tested.
- [x] 3.6 DOM tests over `MemoryBridge`: local card has no Remove; remote rename flows to the sidebar header fixture; the three daemon states render from the projection; Remove confirmation names counts. Cluster gate green. Commit. — `environments/environments.dom.test.tsx` (5 tests, all green): no-Remove-on-local, synthesized live local card, rename-flows-to-second-reader, three daemon states + Reconnect visibility, Remove confirmation counts + machine-untouched.
...
- [x] 6.1 `settings/appearance.tsx`: Scheme as a light/dark/system segmented control (salvage the live `settings.setAppearance` binding from the old file); "system" resolves through `matchMedia` and re-applies when the OS scheme changes (port `appearance-sync.tsx`). *(claims 629, 630)* — **Done ahead of cluster 6** (setAppearance already live; needed a real page to prove the route). Scheme reads `settings.get` and writes `settings.setAppearance` through the data seam (`useCommand`/`useMutation`, never `bridge.invoke`); the `Segmented` atom is the kit's `ToggleGroup`/`Toggle` (autopsy S6 forbids a hand-rolled toggle); a malformed config disables the control (Rule 75); the `ProvenanceChip` shows the scheme's rung. `system`→`matchMedia` re-apply is already served app-wide by `routes/app.tsx`'s `AppearanceSync`. **Remaining in cluster 6:** Theme Pack + Code Theme rows (6.2, B10-absent client prefs needing app-global theme state + code-surface re-highlight), the `dark`-class + `data-rn-theme` stamping (6.3), and the appearance DOM tests (6.4).
