...
#### Scenario: Acknowledging unblocks signing
- **WHEN** the degradations are acknowledged and a hold meeting `holdToSignMs` is completed
- **THEN** the sign callback is called with the previewed bytes

#### Scenario: No ledger imposes no gate
- **WHEN** the sheet is mounted with no ledger (or an empty one) and a hold meeting `holdToSignMs` is completed
- **THEN** the sign callback is called
