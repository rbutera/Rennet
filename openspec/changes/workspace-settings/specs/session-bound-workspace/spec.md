...
#### Scenario: The branch worktree a live session is working in
- **WHEN** a session binds to a branch whose computed path is the workspace another live session is bound to, checked out on that session's branch
- **THEN** the bind fails naming that path, that reference and the session working there, and that worktree's branch, index and working tree are unchanged

#### Scenario: Rennet's own worktree has drifted onto another branch
- **WHEN** a session binds to a branch whose computed path is a worktree Rennet placed that something checked another branch out in, and no live session is working there
- **THEN** the bind checks the reviewed branch out in that worktree and binds to it, creating no second worktree — and if git refuses the checkout to protect uncommitted work, the bind fails with git's own words and that worktree is unchanged
