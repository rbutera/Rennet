...
**3. Retry-once lives in `refreshAndPersist`, guarded by the transient-error test.** After the first `refresh(...)` throws, if `isGitHubNetworkError(err)` is true, log a `network` attempt record and retry exactly once; a second transient failure degrades to `network`. A `GitHubOAuthDeclined` is NOT retried (a decline is deterministic). Rationale: the retry belongs inside the account-lock section so the re-read-and-rotate invariant is preserved; bounding it to one attempt keeps the worst case near two request deadlines.
