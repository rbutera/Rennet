...
On lancelot (WSL, 0.3.14) a `project.detail` PR fetch reported the GitHub token as expired and forced a device-flow re-auth. The refresh path is fully implemented, wired (`resolveGitHubAuth` → `refreshAndPersist` → `refreshGitHubCredential`, with `refresh`+`withLock` bound in `create-server`), and the token store is sound (atomic write-then-rename, fresh reads). Yet the refresh exchange emits **zero logs**, so the failure could only be *inferred*, never observed — and the refresh has never been confirmed to succeed in the field even once. On top of that, a refresh that runs during the daemon's boot connect-timeout storm (`undici` `UND_ERR_CONNECT_TIMEOUT` while github.com is briefly unreachable) gets no retry, so a transient blip at exactly the wrong moment can needlessly drop the session and demand a re-auth.
...
- **Observability**: the daemon logs the refresh exchange — an attempt line, GitHub's HTTP status and (on a 200-with-error) the returned `error` code, whether the rotated pair persisted, and a network-degraded outcome. Token, refresh-token, and any secret value are **never** logged (only shape/length/prefix if anything).
- **Resilience**: the refresh exchange retries **once** on a transient network error (a `UND_ERR_*`/timeout/abort) before degrading, so a boot-storm blip does not drop a live session. A genuine network failure still preserves the stored credential and surfaces reason `network` (never `token-invalid`) — unchanged behavior, now covered by a test.
...
### New Capabilities
- `github-token-refresh`: how the daemon renews an expiring GitHub credential — the refresh exchange, its retry-once-on-transient-network policy, the persistence of the rotated pair, the distinct failure outcomes (network vs declined), and the secret-safe log record of each attempt.
...
- `packages/adapters/src/github-auth.ts` — `refreshAndPersist` / `resolveGitHubAuth`: retry-once on transient network error; emit the secret-safe log records via an injected logger; preserve the existing `network` vs `token-invalid` classification.
