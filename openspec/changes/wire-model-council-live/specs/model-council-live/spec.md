...
The live wire-up that makes the Model Council real end to end: each model-facing pipeline seat is resolved to a mind, executed on the resolved harness (the Claude turn or the Codex port), stamped with honest provenance, and gated by one shared invocation budget across every seat.
