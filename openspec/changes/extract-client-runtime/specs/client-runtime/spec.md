...
The runtime SHALL accept focus/visibility/device-class presence signals from its shell and expose them for transmission to the daemon, so daemon-side notification planning and bandwidth filtering can consume them when that capability lands. Until a daemon consumes presence, the seam SHALL be a well-defined no-op — it SHALL NOT alter protocol traffic.

#### Scenario: presence updates are accepted and inert today

- **WHEN** a shell reports `focused: false` for a review surface
- **THEN** the runtime records the presence state, and current protocol traffic is unchanged
