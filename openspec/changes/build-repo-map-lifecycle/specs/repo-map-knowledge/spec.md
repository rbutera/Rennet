...
The LLM knowledge layer (#14's knowledge half, now v1): evidence-anchored, confidence-labelled learned statements about what a module does, the conventions it embodies, and the reconstructed *why* — served verbatim via `context.knowledge`. This is the only Repo Map layer a model writes; it never enters the structural map or the symbolic surface. Diverges from codeindexer.dev's embedding/vector semantic search + agent-authored memory cards: our knowledge is LLM-reconstructed, evidence-anchored, and invalidated with its snapshot inputs.
