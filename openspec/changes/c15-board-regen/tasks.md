...
- [x] 4.2 Render the derived completion steps "Finalizing generation" and "Composed generation 2" from the real phases in the progress block. Finalization begins only after cross-lens coverage and every lens arrival. **Positive control:** a DOM test over a live round asserts both step labels appear at their phases; they are absent before their phase (not pre-rendered). Lands C3.
