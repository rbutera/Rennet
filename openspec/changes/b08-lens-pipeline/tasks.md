...
- [x] 8.1 `sh -c 'pnpm check'` green — EXIT=0 on its own line, tail shown. (PNPM_CHECK_EXIT=0; all 14 projects format/architecture/licenses/lint/typecheck/test/build; app-ui flake auto-retried green.)
- [x] 8.2 Packet E2E against a real Rennet PR (inject `runTurn` where a live model would run — the pipeline plumbing is pure over the seam): five frozen lens boards in the event log; every patchset hunk either covered by a lens or in some `skippedHunks` set; a deliberately-invalid drafter return exercises the retry ladder and lands as a labeled blemish, not a block. Show the passing run output. (Proven over the injected `runTurn` seam by `lens-pipeline.test.ts` — "drafts all five lenses, writes each board via whiteboard, and emits arrival on freeze" (five frozen boards + per-board arrival events), coverage assertion, round-report-first; retry-ladder→blemish→honest-omission by `validate.test.ts`. No live model call in the gate. 95 board+lens tests pass.)
- [x] 8.3 Positive controls, fail-then-revert with evidence: (a) leave one hunk covered by no lens → the composition every-hunk assert fails; (b) feed a draft carrying code bytes → the R17 parse/lint gate rejects it; (c) break a citation → the citation-resolves rule fires. Revert, re-run green, tree clean. (Each gate broken in source, its test observed RED, reverted, board suite re-run green (80), tree clean — see cluster-8 report.)
