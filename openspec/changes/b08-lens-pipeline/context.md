...
- **Validation loop** (#493): 19 lint rules + schema constraints, ZodError-shaped JSON pointers on one retry channel; drafter returns a patch, passing elements freeze; 4-rung escalation → honest-omission exit (drop element, hunks → `skippedHunks` with reason); retry cap 10; exhaustion = labeled `blemishes[]`, visible, never blocking. Three gates: lint (pre-post-process) → immutability check → composition every-hunk check.
...
Client rendering (C5); exits (B11). Measure the warm-session concurrency cost (engine asset risk 3) before trusting cap 10 — record the measurement in the change.
