# Packaged editor resolution specification

## Purpose
Define how the packaged desktop app resolves an installed editor to an absolute executable and opens a review file at a requested line on macOS, Windows, or WSL.
## Requirements
...
The launch SHALL use `-g <absolute-file>:<line>` and follow `EDITOR_CLIS` priority. If a resolved candidate fails, the launcher SHALL try the next candidate. If all candidates fail, it SHALL use the operating system's file-open command.
...
- **THEN** the OS-level open is attempted for the absolute review file
