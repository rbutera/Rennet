# documentation-library specification

## Purpose

Defines the canonical GitHub-readable documentation library and the separate
static application that renders it as the public documentation site.

## Requirements

### Requirement: Root docs are the canonical documentation library

The repository SHALL keep the only committed copy of reader-facing Markdown
directly under root `docs/`. The library SHALL contain a GitHub-rendered README
that links to the public site and maps every current or planned section.
...
The Astro and Starlight application SHALL live under `apps/docs`. Any generated
content projection SHALL be ignored by Git and derived from the root library
before build or development.
...
Reader-facing documentation SHALL describe how Rennet works now or behavior
that is explicitly planned. It SHALL NOT narrate obsolete designs, completed
migrations, old terminology, or how the current design was reached.

#### Scenario: An obsolete design contains a useful fact

- **WHEN** an audit finds useful material inside an obsolete design or research page
- **THEN** the current fact is moved into the page that owns it and the obsolete narrative is excluded from the documentation library
...
The documentation SHALL map every current Nx application and package, name its
responsibility, and state its allowed in-repository dependencies. The inventory
SHALL be checked against the resolved Nx project graph and package manifests.
...
Page-to-page links, headings, diagrams, and source references SHALL work from
both the canonical Markdown files on GitHub and the rendered site.
...
Every current or planned page, README, ADR, and promoted spec SHALL be checked
against `main` and rewritten where needed to use direct, specific language. The
audit SHALL cover the whole file rather than only lines whose facts changed.
