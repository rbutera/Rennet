...
- **A tool result is billed like a prompt and gets the same byte discipline** (#871). Every rule above governs what a prompt carries; a `tool_result` is charged identically, and it is *worse*, because it sits in the conversation prefix and is re-read on every remaining round trip of that turn — 60–121 of them for a board seat (#867). So a tool result that interpolates a collection declares a cap and carries an honest truncation marker, exactly like a prompt interpolation: `heldIds()` at 20, `POINTER_SAMPLE` at 20, `ISSUE_SAMPLE` at 10, `REMOVED_ID_SAMPLE` at 20. The sighting was a boundary refusal that spelled out every violation one call introduced — 44,295 B in one result — and it only appears on a large host-derived board, which is why no fixture caught it. **Stating this is not enough: the rule has an executable ceiling** in `board-tool-surface.measure.test.ts`, which builds the 1,252-element board it was sighted on and fails when any result exceeds 4 kB or grows more than 10× with the board. A new result that carries a list adds a row there.
