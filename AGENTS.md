...
`packages/types` imports nothing in-repo. `packages/theme` (the shared design-token CSS) imports nothing in-repo. `protocol` may import `types`; `instructions` may import `types`; `core` may import `protocol` and `instructions`; `adapters` may import `core`, `instructions`, and Node; `ui` may import only `types`, `protocol`, `theme`, and browser-safe dependencies; `apps/desktop` is the only Electron package. Spikes are deliberately excluded from the pnpm workspace.
