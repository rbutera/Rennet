...
- The full gate is `pnpm check` (`nx run-many -t format,architecture,licenses,vendor-ledger,lint,typecheck,build`, then `test` for every project except the vendored `t3code-*` ones). The vendored T3 Code suites run as `pnpm t3:test` in CI and on folds only (Rai, 2026-09-03: they add about three minutes cold). Every implementer and reviewer relies on `pnpm check` being the gate command; run it before every push. Use `pnpm nx affected -t lint,typecheck,test,build` for fast branch iteration.
