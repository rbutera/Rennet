...
- **Folds are weekly PRs whose body is the digest** from `pnpm t3:inspect`. A fold also applies the catalog and patch changes the digest's manifest diff shows, in `pnpm-workspace.yaml`. It does **not** run the upstream suites: T3 test their own code and we are not responsible for it (Rai, 2026-09-05). `pnpm t3:test` survives as a manual tool for anyone who wants to run them by hand.
...
- **A board seat is a persistent T3 thread; every other internal turn is a fresh, stateless, `ephemeral: true` session.** The lens seats and the round report run as turns on one thread per seat per generation in the daemon-owned sidecar, so a repair is a follow-up turn carrying only its lint pointers — the base prompt and the draft are already in the conversation (`renderRepairTurn` in `@rennet/prompts`; measured 7,107 → 469 bytes on the repair fixture). Every remaining utility turn — the project scout, the repo map, the delta digest, coverage — is still a cold ephemeral session whose prompt must carry everything, which means everything in it is re-billed on every retry. For those, keep retry prompts to pointers where the design allows, and prefer `resume` (fully wired; the interactive turn loop uses it) when a follow-up turn genuinely continues prior work. A repair turn that re-sends the whole base prompt plus the failing draft costs ~55k tokens to deliver a ~500-token correction, which is exactly why the seats moved.
- **NEVER INLINE CONTEXT. Progressive disclosure, always** (Rai, 2026-09-03, after the Design seat's 241,849-character prompt: an 88k hunk inventory on every seat and 103k of openspec text embedded for one seat to pick one candidate). A prompt names what exists and where; it does not carry it. Anything a seat might need beyond its instructions — the change inventory, blast radius, counterpart hints, discovered artifacts, a prior draft, lint pointers — is written as a file under the project's `.rennet/` folder and the prompt references the path, exactly as it references the checkout, and the agent reads what it decides it needs with its own tools. Those files are purged automatically when the session or chat they belong to is archived or settled. There is no size at which inlining becomes acceptable: a "small" inline block is the seed of the next 240k prompt. The Design lens carries no artifact bundle at all: its prompt says to find the spec for this branch, if there is one (openspec, BMAD, Kiro, grill-me, ADR, superpowers; the commit history is the clue) and draft the Design board from it.
...
- **A tool result is billed like a prompt and gets the same byte discipline** (#871). Every rule above governs what a prompt carries; a `tool_result` is charged identically, and it is *worse*, because it sits in the conversation prefix and is re-read on every remaining round trip of that turn — 60–121 of them for a board seat (#867). So a tool result that interpolates a collection declares a cap and carries an honest truncation marker, exactly like a prompt interpolation: `heldIds()` at 20, `POINTER_SAMPLE` at 20, `ISSUE_SAMPLE` at 10, `REMOVED_ID_SAMPLE` at 20, and for the batching `write_board` verb `CASCADE_SAMPLE` at 40 with `BATCH_NAME_CAP`/`BATCH_SENTENCE_CAP` on each echoed sentence. The sighting was a boundary refusal that spelled out every violation one call introduced — 44,295 B in one result — and it only appears on a large host-derived board, which is why no fixture caught it. **Stating this is not enough: the rule has an executable ceiling** in `board-tool-surface.measure.test.ts`, which builds the 1,252-element board it was sighted on and fails when a PER-CALL result exceeds 4 kB or grows more than 10× with the board. A new per-call result that carries a list adds a row there; `write_board` answers for a whole batch and carries its own, larger, declared envelope.
...
- The full gate is `pnpm check` (`nx run-many -t format,architecture,licenses,vendor-ledger,lint,typecheck,build`, then `test,dogfood-test` for every project except the vendored `t3code-*` ones; `dogfood-test` is the uncacheable suite that reads the live checkout, scheduled beside `test` so it stays off the critical path). The vendored T3 Code suites are **not part of any gate** — not `pnpm check`, not CI, not a fold. They are T3 Code's own tests of T3 Code's own code (Rai, 2026-09-05: "we shouldn't even run the upstream t3 suites, T3 test their own code, we're not responsible for it"). `pnpm t3:test` remains as a manual tool if you ever want to run them. Every implementer and reviewer relies on `pnpm check` being the gate command; run it before every push. Use `pnpm nx affected -t lint,typecheck,test,build` for fast branch iteration.
...
Consequences for the concurrent-agent recipe: run the gate with `CI=true NX_DAEMON=false pnpm check`, and do **not** set `NX_CACHE_DIRECTORY` on its own. A cache entry lives in two stores — the artifact store (`cacheDir`, which honours `NX_CACHE_DIRECTORY`) and the metadata database under `.nx/workspace-data` (which does not, and which Nx deliberately resolves against the main worktree so every lane shares one). **Nx declares a hit from the database and never checks that the artifact store still holds the bytes.** So a lane that isolates one store and not the other writes entries that the *next* lane hits and cannot restore: `t3code-server:build` reports a 100% hit, exits 0, produces no `dist/`, and takes `rennet-desktop:build` down with it (#827 — five fresh worktrees in one evening; when it was measured, 933 of 1616 entries in the shared database were unrestorable). Sharing both stores is correct and lets lanes reuse each other's builds instead of each paying a cold vendored rebuild. The lane-local cache was introduced so one worktree's cleanup could not delete another lane's live artifacts — that cleanup (`rm -rf .nx-isolated`) existed *only* because the lane-local cache existed, and both are gone together, so the hazard it answered is gone with them. The remaining way to delete shared artifacts is a bare `pnpm nx reset` from a worktree, which is already forbidden above, and which `pnpm nx:doctor` now recovers from: the affected lanes rebuild instead of restoring nothing. **Do not reach for `NX_CACHE_DIRECTORY` the next time lanes contend** — it does not isolate the half that decides a hit, and the doctor will refuse the run rather than let it corrupt the shared database again. If a lane genuinely must isolate, isolate both together: `NX_CACHE_DIRECTORY="$PWD/.nx-isolated/cache" NX_WORKSPACE_DATA_DIRECTORY="$PWD/.nx-isolated/workspace-data"`. `pnpm check` runs `scripts/nx-cache-doctor.mjs` first, which refuses a split pair and drops any entry whose recorded bytes are no longer on disk; run it alone with `pnpm nx:doctor` after deleting cache files by hand. Keep `NX_DAEMON=false` on gates, but do **not** carry it into reset: it disables the daemon client, so `pnpm nx reset --onlyDaemon` cannot stop a daemon. A focused `typecheck`-alone run is NOT a sufficient gate (one pass/fail divergence vs `build` on the same command was sighted) — gate with full `pnpm check`, which always includes `build`.
...
- `openspec/` is tracked normally: the global gitignore no longer excludes it and the repo `.gitignore` carries `!openspec/**`. A plain `git add openspec/...` works.
...
1. After every Nx process in this worktree exits, stop only its daemon: `sh -c 'cd <your-worktree> && pnpm nx reset --onlyDaemon && rm -rf .nx-isolated'`. Do **not** set `NX_DAEMON=false` on this reset: that disables the daemon client and skips the daemon stop. Never run bare `pnpm nx reset` from a worktree: its full reset can clear the main checkout's artifact cache and workspace data. `--onlyDaemon` leaves the cache alone; the `rm -rf .nx-isolated` sweeps a lane-local cache if an older recipe left one behind — deleting artifacts under a shared metadata database is exactly what `pnpm nx:doctor` repairs, so run it afterwards if you deleted any.
