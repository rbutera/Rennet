...
function main(argv) {
  ...
  if (!storesArePaired(stores)) {
    process.stderr.write(
      [
        "nx-cache-doctor: stopping, because NX_CACHE_DIRECTORY is set without",
        "NX_WORKSPACE_DATA_DIRECTORY.",
        "",
        "IF YOU WERE GIVEN A GATE COMMAND CONTAINING NX_CACHE_DIRECTORY, THAT",
        "BRIEFING IS THE OLD RECIPE. It was withdrawn in issue #827. Run instead:",
        "",
        "  CI=true NX_DAEMON=false pnpm check",
        "",
        "Nothing else about the gate changed, and you do not need to clean up a",
        "lane-local cache afterwards, because there no longer is one.",
        "",
        "Why: an Nx cache entry lives in two stores, and only one of them moves.",
        `  artifacts     ${stores.artifacts}`,
        "                (honours NX_CACHE_DIRECTORY)",
        `  metadata (db) ${stores.data}`,
        "                (ignores it; Nx shares this with the main worktree)",
        "",
        "Nx decides a cache hit from the metadata database and never checks that",
        "the artifact store still holds the bytes. Move one store without the",
        "other and Nx reports hits it cannot restore: t3code-server:build goes",
        "green with no dist/, and rennet-desktop:build then fails staging a",
        "sidecar bundle that was never built.",
        "",
        "If you genuinely need an isolated cache, isolate BOTH stores together:",
