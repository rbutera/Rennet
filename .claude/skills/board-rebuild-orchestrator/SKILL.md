...
1. **Gate check**: the packet's Blocked-by entries are all `passes: true` in `BUILD-STATUS.json`. Cross-track gates: B4 needs Track A's npm alpha (A5); Track C proper needs B3. One workstream in flight per **merge surface**, not per track: before co-dispatching two workstreams (same track or not), compare their packets' file footprints — the packages and directories each `context.md` touches. Disjoint footprints may run in parallel; overlapping ones serialize. Implementation parallelizes but **landing stays serial**: merge one PR to `main` at a time; the others rebase before their review round. Keep the critical-path chain (currently B5 → B8 → B9 → B11) never-idle and fill spare slots from Track C. Run **4 active implementation worktrees** whenever 4 gate-satisfied, footprint-disjoint workstreams exist — fewer only when the dependency graph genuinely can't supply them or the host shows real memory pressure. Review seats (diff + packet, read-only) don't count against the cap. The 16 GB host tolerates this only because worktree/daemon cleanup on merge is enforced — the historical ~8 GB swap incident came from stale uncleaned nx daemons, not from live width.
