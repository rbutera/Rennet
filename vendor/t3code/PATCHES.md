...
| `apps/server/src/provider/Layers/ClaudeAdapter.test.ts` | Covers the new adapter behaviours above: the schema becomes the SDK's `outputFormat`, no schema means no output format, a turn asking for a different contract than its session is refused, and the spawn hook terminates a child whose stdin breaks instead of letting the EPIPE escape. | yes | pending |
