...
| `apps/server/src/provider/Layers/ClaudeAdapter.test.ts` | Covers the three new output-schema behaviours of the adapter above: the schema becomes the SDK's `outputFormat`, no schema means no output format, and a turn asking for a different contract than its session is refused. | yes | pending |
